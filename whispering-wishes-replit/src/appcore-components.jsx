  ...
  useEffect(() => {
    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        // === FLOOR ELECTRICITY — sparse erratic arcs between nearby swords ===
        if (swords.length > 1) {
          ctx.save();
          ctx.globalCompositeOperation = 'lighter';
          ctx.lineCap = 'round'; ctx.lineJoin = 'round';
          var fet = cloudTime * 0.15;
          // Erratic timing — seed changes slowly and irregularly
          var fSeedT = Math.floor(fet * 0.08);
          // Only arc from every 6th sword — keeps it sparse
          for (var fi2 = 0; fi2 < swords.length; fi2 += 6) {
            var fSw = swords[fi2];
            // Erratic phase — multiple overlapping slow sins create irregular flicker
            var fP1 = Math.sin(fet * 0.03 + fi2 * 2.7);
            var fP2 = Math.sin(fet * 0.017 + fi2 * 4.1);
            var fP3 = Math.sin(fet * 0.009 + fi2 * 1.3);
            var fPhase2 = fP1 * fP2 * fP3;
            if (fPhase2 < 0.3) continue;
            var fAlpha2 = (fPhase2 - 0.3) * 1.2;
            // Find nearest sword that isn't self
            var fBestDist = 99999, fTarget = -1;
            for (var fti = 0; fti < swords.length; fti++) {
              if (fti === fi2) continue;
              var ftdx = swords[fti].scrX - fSw.scrX, ftdy = swords[fti].scrY - fSw.scrY;
              var ftd = Math.sqrt(ftdx * ftdx + ftdy * ftdy);
              if (ftd > 20 && ftd < fBestDist) { fBestDist = ftd; fTarget = fti; }
            }
            if (fTarget < 0 || fBestDist > W * 0.35) continue;
            var fTw = swords[fTarget];
            var fx0 = fSw.scrX, fy0 = fSw.scrY;
            var fx1 = fTw.scrX, fy1 = fTw.scrY;
            var fdx = fx1 - fx0, fdy = fy1 - fy0;
            var fDist = Math.sqrt(fdx * fdx + fdy * fdy);
            // Reseed per arc — changes with fSeedT for erratic reshaping
            ...
            for (var fsi2 = 1; fsi2 <= fNSeg2; fsi2++) {
              ...
              fmpy2.push(fy0 + fdy * ft2 + (faRng() - 0.5) * fDist * 0.15);
            }
            // Main arc — same colors as cloud electricity
            ctx.beginPath();
            ...
            for (var fbi2 = 0; fbi2 < fNBr2; fbi2++) {
              ...
              var fbrAng2 = Math.atan2(fdy, fdx) + (faRng() - 0.5) * 1.5;
              var fbrLen2 = fDist * (0.15 + faRng() * 0.25);
              var fbrSegs2 = 3 + (fbi2 % 2);
              ctx.beginPath();
              ctx.moveTo(fmpx2[fbrIdx2], fmpy2[fbrIdx2]);
              for (var fbsi2 = 1; fbsi2 <= fbrSegs2; fbsi2++) {
                ctx.lineTo(
                  fmpx2[fbrIdx2] + (fbsi2 / fbrSegs2) * fbrLen2 * Math.cos(fbrAng2) + (faRng() - 0.5) * fbrLen2 * 0.25,
