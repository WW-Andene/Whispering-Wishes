    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        function drawWeapon(sx, sy, sh, sbw, tilt, wp) {
          ...
          // === DRAW BLADE ===
          // Z-rotation only affects width (foreshortening) and shading (gradient).
          // Shape stays symmetric — no vertex shifting which causes distortion.
          ...
          ctx.lineTo(tipW + bSkew * 0.6, -1);
          ctx.closePath();
          // Across-blade gradient shows which side faces the light
          if (Math.abs(zSin) > 0.05 && effBW > 0.5) {
            const blGrad = ctx.createLinearGradient(-effBW, 0, effBW, 0);
            const darkSide = zSin > 0 ? 0 : 1;
            const litF = 1.3, dkF = 0.5;
