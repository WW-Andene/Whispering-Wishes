    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        function drawWeapon(sx, sy, sh, sbw, tilt, wp) {
          ...
          // blade / handle = φ (1.618). Total = blade + handle.
          // blade = 62%, guard = 3%, grip = 25%, pommel = 10%
          const bladeH = sh * 0.62;
          const guardBarH = sh * 0.03;
          const gripH = sh * 0.25;
          const pommelH = sh * 0.10;

          // Blade width = 1/16 of blade length
          const halfW = Math.max(1, bladeH / 16);
          const halfT = Math.max(1.5, halfW * 0.10); // blade depth

          // Grip diameter = blade width / 1.5
          const gripHW = halfW / 1.5;
          const gripHT = gripHW;

          // Guard: each arm = blade / 8. Bar thickness = grip diameter.
          const guardArmHW = bladeH / 8;
          const guardBarHT = gripHW; // guard depth = grip diameter

          // Pommel ≈ blade width
          const pomR = halfW * wt.pomM;

          const rainGuardH = sh * 0.0; // not using rain guard
