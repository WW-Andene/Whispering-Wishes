    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        // === CENTERPIECE GLADIUS ===
        {
          ...
          const gScale = H * 0.0045;
          // Gladius planted slightly tilted
          ctx.translate(gx, gy);
          ctx.scale(gScale, -gScale); // flip so blade points up
          ctx.rotate(0.04); // very slight lean

          // Proportions (in unit space, ~100 units tall)
          const bH = 58, bW = 5.5; // blade height, half-width at base
          const taperStart = 18; // where blade starts narrowing to tip
          const guardH2 = 2.5, guardHW = 12; // guard thickness, half-width
          const gripH2 = 20, gripHW = 2.8; // grip height, half-width
          const pomR = 4; // pommel radius
          const ringH = 1.2; // brass ring height

          // Lighting
          const gLit = 0.7; // centerpiece is well-lit
          const leftLit = gx > sunX;
          const litCol = `rgb(200,192,175)`;
          const darkCol = `rgb(85,80,72)`;
          const ridgeCol = `rgba(230,225,210,0.5)`;
          const gripCol = `rgb(40,22,14)`;
          const gripHiCol = `rgb(60,35,22)`;
          const metalCol = `rgb(140,132,118)`;
          const brassCol = `rgb(180,150,60)`;

          // Blade left half — parallel sides then taper
          ctx.fillStyle = leftLit ? litCol : darkCol;
          ctx.beginPath();
          ctx.moveTo(0, bH); // tip
          ctx.lineTo(-bW * 0.3, taperStart); // taper from tip
          ctx.lineTo(-bW, taperStart); // full width
          ctx.lineTo(-bW, 0); // blade base
          ctx.lineTo(0, 0);
          ctx.closePath();
          ctx.fill();

          // Blade right half
          ctx.fillStyle = leftLit ? darkCol : litCol;
          ctx.beginPath();
          ctx.moveTo(0, bH);
          ctx.lineTo(bW * 0.3, taperStart);
          ctx.lineTo(bW, taperStart);
          ctx.lineTo(bW, 0);
          ctx.lineTo(0, 0);
          ctx.closePath();
          ctx.fill();
          ...
          ctx.beginPath();
          ctx.ellipse(0, -guardH2 / 2, guardHW, guardH2, 0, 0, Math.PI * 2);
          ctx.fill();

          // Brass ring above grip
          ctx.fillStyle = brassCol;
          ctx.fillRect(-gripHW * 1.1, -guardH2 - ringH, gripHW * 2.2, ringH);

          // Grip — turned wood shape (wider at ends, narrow in middle)
          ctx.fillStyle = gripCol;
          ctx.beginPath();
          ctx.moveTo(-gripHW, -guardH2 - ringH);
          ctx.quadraticCurveTo(-gripHW * 0.6, -guardH2 - gripH2 * 0.3, -gripHW * 0.85, -guardH2 - gripH2 * 0.5);
          ctx.quadraticCurveTo(-gripHW * 0.55, -guardH2 - gripH2 * 0.7, -gripHW * 0.8, -guardH2 - gripH2);
          ctx.lineTo(gripHW * 0.8, -guardH2 - gripH2);
          ctx.quadraticCurveTo(gripHW * 0.55, -guardH2 - gripH2 * 0.7, gripHW * 0.85, -guardH2 - gripH2 * 0.5);
          ctx.quadraticCurveTo(gripHW * 0.6, -guardH2 - gripH2 * 0.3, gripHW, -guardH2 - ringH);
          ctx.closePath();
          ctx.fill();
          // Grip highlight
          ctx.fillStyle = gripHiCol;
          ctx.beginPath();
          ctx.moveTo(-gripHW * 0.15, -guardH2 - ringH);
          ctx.quadraticCurveTo(-gripHW * 0.1, -guardH2 - gripH2 * 0.5, -gripHW * 0.15, -guardH2 - gripH2);
          ctx.lineTo(gripHW * 0.15, -guardH2 - gripH2);
          ctx.quadraticCurveTo(gripHW * 0.1, -guardH2 - gripH2 * 0.5, gripHW * 0.15, -guardH2 - ringH);
          ctx.closePath();
          ctx.fill();

          // Brass ring below pommel
          ctx.fillStyle = brassCol;
          const pomY = -guardH2 - gripH2;
          ctx.fillRect(-gripHW * 1.0, pomY - ringH, gripHW * 2.0, ringH);

          // Pommel — round ball
          ctx.fillStyle = metalCol;
          ctx.beginPath();
          ctx.arc(0, pomY - ringH - pomR * 0.8, pomR, 0, Math.PI * 2);
          ctx.fill();
          // Pommel highlight
          ctx.fillStyle = `rgba(200,195,180,0.4)`;
          ctx.beginPath();
          ctx.arc(-pomR * 0.25, pomY - ringH - pomR * 1.1, pomR * 0.35, 0, Math.PI * 2);
          ctx.fill();
