        ...
        for (const s of swords) {
          ...
          if (isGladius) {
            ...
            for (var _gci2 = 0; _gci2 < _gnc; _gci2++) {
              ...
              var _gdiag = gBW * 2 * (1.5 + _gcr() * 2.5);
              var _gdir = _gcr() > 0.5 ? 1 : -1;
              var _gkinkX = (_gcr() * 0.6 - 0.3) * _gw2;
              var _gkinkB = (_gcr() - 0.5) * _gdiag * 0.4;
              _gcracks.push([
                { x: -_gw2 * 1.3, y: _gy - _gdir * _gdiag * 0.5 },
                { x: _gkinkX, y: _gy + _gkinkB },
                { x: _gw2 * 1.3, y: _gy + _gdir * _gdiag * 0.5 }
              ]);
            }
            var _gfdx = [0], _gfdy = [0], _gfr = [0];
            for (var _gfi = 0; _gfi < _gnc; _gfi++) {
              _gfdx.push((_gcr() - 0.5) * gBW * 0.6);
          ...
          for (var _ci2 = 0; _ci2 < _nc; _ci2++) {
            ...
            // Steep diagonal — 2-4x blade width makes sharp angular cuts
            var _diag = bladeW * (1.5 + _cr() * 2.5);
            var _dir = _cr() > 0.5 ? 1 : -1;
            // Sharp V zigzag: left edge, sharp kink near center, right edge
            var _kinkX = (_cr() * 0.6 - 0.3) * _w;
            var _kinkBias = (_cr() - 0.5) * _diag * 0.4;
            _cracks.push([
              { x: -_w * 1.3, y: _y - _dir * _diag * 0.5 },
              { x: _kinkX, y: _y + _kinkBias },
              { x: _w * 1.3, y: _y + _dir * _diag * 0.5 }
            ]);
          }
