      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        if (!skyFrames && !skyPrerendering && skyCacheChecked && !skyCacheLoading) {
          ...
          skyPreRenderQueue = {
            ...
            frameIdx: 0, totalFrames: totalSkyFrames,
            W: W, H: H, sunX: sunX, sunY: sunY, sunR: sunR,
            sW: Math.ceil(W * skyScale), sH: Math.ceil(H * skyScale),
            frames: []
          };
        }

        // Staggered pre-render: render 2 frames per real frame to avoid blocking
        if (skyPreRenderQueue && !skyFrames) {
          var q = skyPreRenderQueue;
          var framesPerTick = 2;
          for (var fi = 0; fi < framesPerTick && q.frameIdx < q.totalFrames; fi++) {
            ...
            fCtx.save();
            fCtx.scale(skyScale, skyScale);
            // Advance clouds for this frame
            for (var ci = 0; ci < q.clouds.length; ci++) {
              var cloud = q.clouds[ci], angSpeed = cloud.orbitSpeed * 0.025;
              cloud.angle += angSpeed;
            }
            // Draw all clouds
            for (var di = 0; di < q.clouds.length; di++) {
              var cl = q.clouds[di];
              var ca = cl.angle, flatR = cl.orbitFlatten || 0.7;
              var cx2 = cl.sunX + Math.cos(ca) * cl.orbitDist;
              var cy2 = cl.sunY + Math.sin(ca) * cl.orbitDist * flatR;
              var bk = cl.baked; var bkSrc = bk && (bk.bitmap || bk.canvas); if (!bkSrc) continue;
              var m2 = Math.max(bk.w, bk.h) * 1.5;
              if (cx2 + bk.ox > q.W + m2 || cx2 + bk.ox + bk.w < -m2 || cy2 + bk.oy > q.H + m2 || cy2 + bk.oy + bk.h < -m2) continue;
              var vx2 = -Math.sin(ca) * cl.orbitDist * angSpeed, vy2 = Math.cos(ca) * cl.orbitDist * flatR * angSpeed;
              var spd = Math.sqrt(vx2 * vx2 + vy2 * vy2);
              var dW = bk.w, dH = bk.h;
              if (spd > 0.01) {
                var sA = 1 + Math.min(0.6, spd * 0.25), sqA = 1 / Math.sqrt(sA);
                var cSk = Math.max(-0.3, Math.min(0.3, angSpeed * cl.orbitDist * 0.0004));
                var vA = Math.atan2(vy2, vx2);
                var ccx2 = cx2 + bk.ox + dW * 0.5, ccy2 = cy2 + bk.oy + dH * 0.5;
                fCtx.save(); fCtx.translate(ccx2, ccy2); fCtx.rotate(vA);
                fCtx.transform(sA, cSk, 0, sqA, 0, 0);
                fCtx.rotate(-vA); fCtx.drawImage(bkSrc, -dW * 0.5, -dH * 0.5, dW, dH); fCtx.restore();
              } else {
                fCtx.drawImage(bkSrc, cx2 + bk.ox, cy2 + bk.oy, dW, dH);
              }
            }
            // God rays
            ...
            for (var ri2 = 0; ri2 < 18; ri2++) {
              ...
              var rayLen = (q.H - q.sunY) * (0.8 + rayRng() * 0.4), rayW = q.sunR * (0.15 + rayRng() * 0.5);
              var rex = q.sunX + Math.cos(rayAngle) * rayLen, rey = q.sunY + Math.sin(rayAngle) * rayLen;
              var rG = fCtx.createLinearGradient(q.sunX, q.sunY, rex, rey);
              var rA = 0.02 + rayRng() * 0.04;
              rG.addColorStop(0, 'rgba(255,230,140,' + (rA * 1.5) + ')');
              rG.addColorStop(0.2, 'rgba(255,200,90,' + rA + ')');
              rG.addColorStop(0.6, 'rgba(255,150,50,' + (rA * 0.35) + ')');
              rG.addColorStop(1, 'rgba(255,100,20,0)');
              fCtx.fillStyle = rG; fCtx.beginPath();
              var pX = -Math.sin(rayAngle), pY = Math.cos(rayAngle);
              fCtx.moveTo(q.sunX + pX * rayW * 0.15, q.sunY + pY * rayW * 0.15);
              fCtx.lineTo(q.sunX - pX * rayW * 0.15, q.sunY - pY * rayW * 0.15);
              fCtx.lineTo(rex - pX * rayW * 2.5, rey - pY * rayW * 2.5);
              fCtx.lineTo(rex + pX * rayW * 2.5, rey + pY * rayW * 2.5);
              fCtx.closePath(); fCtx.fill();
            }
            fCtx.restore();
            fCtx.restore(); // undo scale
            q.frames.push(fCvs);
            q.frameIdx++;
          }
          // Done pre-rendering?
        ...
        if (skyFrames && skyFrames.length > 0) {
          ctx.drawImage(skyFrames[skyFrameIdx % skyFrames.length], 0, 0, W, H);
          skyFrameIdx++;
        }
