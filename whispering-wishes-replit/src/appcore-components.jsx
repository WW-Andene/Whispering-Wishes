    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        if (!skyBuffer || skyBuffer.width !== halfW || skyBuffer.height !== halfH) {
          skyBuffer = document.createElement('canvas'); skyBuffer.width = halfW; skyBuffer.height = halfH;
        }
        var sCtx = skyBuffer.getContext('2d');
        sCtx.clearRect(0, 0, halfW, halfH);
        sCtx.save();
        sCtx.scale(0.5, 0.5);
        ...
        // Advance + draw clouds with velocity stretch
        var maxViewDist = Math.sqrt(W * W + H * H) * 0.5 + 200;
        for (var di = 0; di < sceneClouds.length; di++) {
          var cloud = sceneClouds[di], angSpeed = cloud.orbitSpeed * 0.025;
          cloud.angle += angSpeed;
          if (cloud.orbitDist > maxViewDist) continue;
          ...
          if (speed > 0.01) {
            ...
            sCtx.save(); sCtx.translate(ccx, ccy); sCtx.rotate(vAngle);
            sCtx.transform(stretchAmt, centSkew, 0, squeezeAmt, 0, 0);
            sCtx.rotate(-vAngle); sCtx.drawImage(bkSrc, -drawW * 0.5, -drawH * 0.5, drawW, drawH); sCtx.restore();
          } else {
            sCtx.drawImage(bkSrc, cx2 + bk.ox, cy2 + bk.oy, drawW, drawH);
          }
        }
        // God rays
