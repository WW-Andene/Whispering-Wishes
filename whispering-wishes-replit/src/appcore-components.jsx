    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        for (const s of swords) {
          ...
          // === LONGSWORD — shattered blade: diagonal cuts, damaged edges, shards ===
          ...
          for (var _ni = 0; _ni < _nNotches; _ni++) {
            ...
            if (_cr() > 0.35) _notchesR.push({ y: _ny + (_cr()-0.5)*_nh, d: _nd, h: _nh });
          }
          ...
          // Notch edge helper
          var _drawNotch = function(notches, side, fromY, toY) {
            var _x = side * _hw;
            var _pY = fromY;
            for (var _i = 0; _i < notches.length; _i++) {
              var _n = notches[_i];
              if (_n.y <= _pY || _n.y + _n.h > toY) continue;
              ctx.lineTo(_x, _n.y);
              ctx.lineTo(_x + (-side) * _n.d, _n.y + _n.h * 0.4);
              ctx.lineTo(_x, _n.y + _n.h);
              _pY = _n.y + _n.h;
            }
          };
          // No translate for separation — inset handles it. Tiny X drift only.
          var _driftX = [0];
          for (var _oi = 0; _oi < _nCuts; _oi++) _driftX.unshift((_cr() > 0.5 ? 1 : -1) * _cr() * 3);
          // Draw pieces
          for (var _si = 0; _si <= _nCuts; _si++) {
            var _topCut = _si > 0 ? _cuts[_si - 1] : null;
            var _botCut = _si < _nCuts ? _cuts[_si] : null;
            ctx.save();
            ctx.translate(_driftX[_si] || 0, 0);
            // Inset 4px from shared boundary — 8px total gap between pieces
            var _tL = _topCut ? _cutY(_topCut, -_hw) + 4 : tipEnd;
            var _tR = _topCut ? _cutY(_topCut, _hw) + 4 : tipEnd;
            var _bL = _botCut ? _cutY(_botCut, -_hw) - 4 : guardH;
            var _bR = _botCut ? _cutY(_botCut, _hw) - 4 : guardH;
            // Left half
            ctx.fillStyle = _gL();
            ctx.beginPath();
            if (!_topCut) {
              ...
              ctx.moveTo(0, _cutY(_topCut, 0) + 4);
              ctx.lineTo(_topCut.mx < 0 ? _topCut.mx : -_hw * 0.3, _cutY(_topCut, _topCut.mx < 0 ? _topCut.mx : -_hw * 0.3) + 4);
              ctx.lineTo(-_hw, _tL);
            }
            ...
            if (!_botCut) {
              ...
              ctx.lineTo(_botCut.mx < 0 ? _botCut.mx : -_hw * 0.3, _cutY(_botCut, _botCut.mx < 0 ? _botCut.mx : -_hw * 0.3) - 4);
              ctx.lineTo(0, _cutY(_botCut, 0) - 4);
            }
            ctx.closePath(); ctx.fill();
            // Right half
            ctx.fillStyle = _gR();
            ...
            var _rT = _topCut ? _cutY(_topCut, 0) + 5 : -bladeH + 1;
            ...
            if (_topCut) {
              ...
              ctx.lineWidth = 1;
              ctx.beginPath();
              ctx.moveTo(-_hw, _cutY(_topCut, -_hw) + 4);
              ctx.lineTo(_topCut.mx, _topCut.yM + 4);
              ctx.lineTo(_hw, _cutY(_topCut, _hw) + 4);
              ctx.stroke();
            }
            if (_botCut) {
              ctx.strokeStyle = 'rgba(0,0,0,0.85)';
              ctx.lineWidth = 1;
              ctx.beginPath();
              ctx.moveTo(-_hw, _cutY(_botCut, -_hw));
              ctx.lineTo(_botCut.mx, _botCut.yM);
              ctx.lineTo(_hw, _cutY(_botCut, _hw));
              ctx.stroke();
            }
            ctx.restore();
          }
          // Floating shards near damaged edges — point OUTWARD, visible size
          ...
          for (var _sli = 0; _sli < _notchesL.length; _sli++) {
            ...
            var _sg = Math.max(3, _hw * 0.3 + _cr() * 4);
            var _sx = -_hw - _sg;
            var _sw = Math.max(3, _sn.d * (0.6 + _cr() * 0.5));
            var _sh = Math.max(4, _sn.h * (0.5 + _cr() * 0.6));
            ctx.beginPath();
            ctx.moveTo(_sx, _sn.y);
            ctx.lineTo(_sx - _sw * 0.7, _sn.y + _sh * 0.4);
            ctx.lineTo(_sx - _sw * 0.1, _sn.y + _sh);
            ctx.closePath(); ctx.fill();
          }
          for (var _sri = 0; _sri < _notchesR.length; _sri++) {
            var _snr = _notchesR[_sri];
            var _sgR = Math.max(3, _hw * 0.3 + _cr() * 4);
            var _sxr = _hw + _sgR;
            var _swr = Math.max(3, _snr.d * (0.6 + _cr() * 0.5));
            var _shr = Math.max(4, _snr.h * (0.5 + _cr() * 0.6));
            ctx.beginPath();
            ctx.moveTo(_sxr, _snr.y);
            ctx.lineTo(_sxr + _swr * 0.7, _snr.y + _shr * 0.4);
            ctx.lineTo(_sxr + _swr * 0.1, _snr.y + _shr);
            ctx.closePath(); ctx.fill();
          }
