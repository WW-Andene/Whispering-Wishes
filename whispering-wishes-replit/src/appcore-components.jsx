  ...
  useEffect(() => {
    ...
    const draw = (t) => {
      ...
      // ===== BATTLEGROUND — ground-plan projected sword field =====
      {
        ...
        // ================================================================
        // LIGHTWEIGHT SWORD — same look, fewer polygons/layers
        // ================================================================
        function drawWeaponLite(sx, sy, sh, sbw, tilt, wp) {
          if (sh < 1.5 || sbw < 0.2) return;
          const wt = wp.type || weaponTypes[0];
          const theta = wp.zRot || 0;
          const cosA = Math.cos(theta);
          const sinA = Math.sin(theta);
          ...
          // Dimensions (same as full version)
          const bladeH = sh * 8 / 11;
          const handleH = sh * 3 / 11;
          const mod = bladeH / 8;
          const halfW = mod * 0.25 * wt.bwM;
          const halfT = Math.max(1.5, halfW * 0.10);
          const gripHW = halfW * 0.5;
          const leatherHW = halfW * 0.65;
          const pommelH = mod * 1.0;
          const guardBarH = mod * 0.15;
          const gripH = handleH - pommelH - guardBarH;
          const gripHT = gripHW;
          const guardArmHW = mod * 1.0;
          const pomR = gripHW * wt.pomM;
          const bladeTop = -bladeH;
          const guardTop = bladeTop - guardBarH;
          const gripBot = guardTop;
          const gripTop = gripBot - gripH;
          ...
          const bladeMat = materials[wt.bladeMat] || materials.steel;
          const handleMat = materials[wt.handleMat] || materials.wood;
          const bnx = sinA, bnz = -cosA;
          const bladeLight = calcLight(wp.wx, 2.0, wp.wz, bnx, 0, bnz, bladeMat);
          const handleLight = calcLight(wp.wx, 3.5, wp.wz, bnx, 0, bnz, handleMat);
          const dk = wp.darkness || 0;
          const dkMul = Math.pow(1 - dk, 2.2);
          const blR = Math.round(bladeLight.r * dkMul);
          const blG = Math.round(bladeLight.g * dkMul);
          ...
          // === BLADE — straight tip sides, no curved segments ===
          const taperY = -bladeH * 0.10;
          const tipT = Math.max(0.3, halfT * 0.3);
          const edgeW = halfW * 2 / 7;
          const innerHW = halfW - edgeW;
          ...
          // Draw blade — straight tip (no curve segments)
          if (backVis) fillPoly([bTL, bTR, bMR, bTip, bML], backCol);
          if (rbVis) { fillPoly([eTR, eMR, bMR, bTR], bBevelCol); fillPoly([eMR, eTip, bTip, bMR], bBevelCol); }
          if (lbVis) { fillPoly([bTL, bML, eML, eTL], bBevelCol); fillPoly([bML, bTip, eTip, eML], bBevelCol); }
          if (rfVis) { fillPoly([fTR, fMR, eMR, eTR], fBevelCol); fillPoly([fMR, fTip, eTip, eMR], fBevelCol); }
          if (lfVis) { fillPoly([eTL, eML, fML, fTL], fBevelCol); fillPoly([eML, eTip, fTip, fML], fBevelCol); }
          if (frontVis) fillPoly([fTL, fTR, fMR, fTip, fML], frontCol);
          if (rbVis || rfVis) fillPoly([fTR, bTR, eTR], sideCol);
          if (lbVis || lfVis) fillPoly([bTL, fTL, eTL], sideCol);

          // === GUARD — fewer samples, single depth layer ===
          const gCenterY = bladeTop - guardBarH * 0.5;
          const gBarDepth = gripHT;
          ...
          const gcHW = halfW * 1.1;
          {
            ...
            const ghh = guardBarH * 0.5;
            const tipFlare = gFlareH * 0.5;
            const tipOuterX = guardArmHW + gTipW;
            const tipInnerX = guardArmHW;
            const vOuterX = halfW * 1.15;
            const vInnerX = halfW * 0.2;
            const vOver = ghh * 0.6;
            const armBow = ghh * 0.45;
            const NS = 4; // fewer samples

            const gSamplesTop = [];
            gSamplesTop.push({ x: -tipOuterX, y: gCenterY - tipFlare });
            gSamplesTop.push({ x: -tipInnerX, y: gCenterY - tipFlare * 0.6 });
            for (let i = 1; i <= NS; i++) gSamplesTop.push(cBez(-tipInnerX, gCenterY - tipFlare * 0.6, -tipInnerX * 0.7, gCenterY - ghh - armBow, -vOuterX - gcHW * 0.2, gCenterY - ghh - armBow, -vOuterX, gCenterY - ghh, i / NS));
            for (let i = 1; i <= NS; i++) gSamplesTop.push(qBez(-vOuterX, gCenterY - ghh, -vInnerX * 1.5, gCenterY - ghh - vOver, -vInnerX, gCenterY - ghh - vOver, i / NS));
            for (let i = 1; i <= NS; i++) gSamplesTop.push(qBez(vInnerX, gCenterY - ghh - vOver, vInnerX * 1.5, gCenterY - ghh - vOver, vOuterX, gCenterY - ghh, i / NS));
            for (let i = 1; i <= NS; i++) gSamplesTop.push(cBez(vOuterX, gCenterY - ghh, vOuterX + gcHW * 0.2, gCenterY - ghh - armBow, tipInnerX * 0.7, gCenterY - ghh - armBow, tipInnerX, gCenterY - tipFlare * 0.6, i / NS));
            gSamplesTop.push({ x: tipOuterX, y: gCenterY - tipFlare });

            const gSamplesBot = gSamplesTop.map(p => ({ x: p.x, y: gCenterY + (gCenterY - p.y) }));

            // Single front/back layer (no barrel curve)
            ...
            if (backVis) {
              const bp = [...gTopB, ...gBotB.slice().reverse()];
              ctx.beginPath(); ctx.moveTo(bp[0].x, bp[0].y);
              for (let i = 1; i < bp.length; i++) ctx.lineTo(bp[i].x, bp[i].y);
              ctx.closePath(); ctx.fillStyle = guardSideCol; ctx.fill();
            }
            // Single depth quads (no multi-layer)
            for (let i = 0; i < gTopF.length - 1; i++) fillPoly([gTopF[i], gTopF[i + 1], gTopB[i + 1], gTopB[i]], guardSideCol);
            for (let i = 0; i < gBotF.length - 1; i++) fillPoly([gBotF[i], gBotF[i + 1], gBotB[i + 1], gBotB[i]], guardSideCol);
            ...
            if (frontVis) {
              const fp = [...gTopF, ...gBotF.slice().reverse()];
              ctx.beginPath(); ctx.moveTo(fp[0].x, fp[0].y);
              for (let i = 1; i < fp.length; i++) ctx.lineTo(fp[i].x, fp[i].y);
              ctx.closePath(); ctx.fillStyle = guardCol; ctx.fill();
            }
          }

          // === GRIP — simple rect + leather trapezoid, no ridge details ===
          {
            const gripMidY = (gripTop + gripBot) * 0.5;
            const leatherTop = gripBot;
            const leatherBot = gripMidY;
            const cylShade = Math.abs(cosA);
            const cylR = Math.round(hdR * (0.5 + cylShade * 0.4));
            const cylG = Math.round(hdG * (0.5 + cylShade * 0.4));
            const cylB = Math.round(hdB * (0.5 + cylShade * 0.4));
            ...
            const cylHi = 'rgb(' + Math.min(255, cylR + 20) + ',' + Math.min(255, cylG + 20) + ',' + Math.min(255, cylB + 20) + ')';
            const cylLo = 'rgb(' + Math.round(cylR * 0.55) + ',' + Math.round(cylG * 0.55) + ',' + Math.round(cylB * 0.55) + ')';
            if (sinA > 0) { cylGrad.addColorStop(0, cylHi); cylGrad.addColorStop(0.45, cylHi); cylGrad.addColorStop(1, cylLo); }
            else { cylGrad.addColorStop(0, cylLo); cylGrad.addColorStop(0.55, cylHi); cylGrad.addColorStop(1, cylHi); }
            ctx.beginPath(); ctx.rect(-gripHW, gripTop, gripHW * 2, gripBot - gripTop);
            ctx.fillStyle = gripSideCol; ctx.fill();
            ctx.beginPath(); ctx.rect(-gripHW, gripTop, gripHW * 2, gripBot - gripTop);
            ctx.fillStyle = cylGrad; ctx.fill();

            // Leather — simple trapezoid, no wrap ridges
            const lR = Math.round(hdR * 0.25), lG = Math.round(hdG * 0.2), lB = Math.round(hdB * 0.18);
            const leatherGrad = ctx.createLinearGradient(-leatherHW, 0, leatherHW, 0);
            const lHi = 'rgb(' + Math.min(255, lR + 12) + ',' + Math.min(255, lG + 8) + ',' + Math.min(255, lB + 6) + ')';
            const lLo = 'rgb(' + Math.round(lR * 0.4) + ',' + Math.round(lG * 0.4) + ',' + Math.round(lB * 0.4) + ')';
            if (sinA > 0) { leatherGrad.addColorStop(0, lHi); leatherGrad.addColorStop(0.4, lHi); leatherGrad.addColorStop(1, lLo); }
            else { leatherGrad.addColorStop(0, lLo); leatherGrad.addColorStop(0.6, lHi); leatherGrad.addColorStop(1, lHi); }
            const lWideHW = leatherHW, lNarrowHW = gripHW + 1;
            ctx.beginPath();
            ctx.moveTo(-lWideHW, leatherTop); ctx.lineTo(lWideHW, leatherTop);
            ctx.lineTo(lNarrowHW, leatherBot); ctx.lineTo(-lNarrowHW, leatherBot);
            ctx.closePath(); ctx.fillStyle = leatherGrad; ctx.fill();
          }

          // === POMMEL — fewer samples, no cutout/gradient overlay ===
          if (pomR > 0.3) {
            const pomCol = 'rgb(' + Math.round(hdR * 0.85) + ',' + Math.round(hdG * 0.85) + ',' + Math.round(hdB * 0.85) + ')';
            const pomLoCol = 'rgb(' + Math.round(hdR * 0.5) + ',' + Math.round(hdG * 0.5) + ',' + Math.round(hdB * 0.5) + ')';
            const pomDepth = gripHT;
            const pomHW = pomR * 1.8, pomH = pomR * 2.2;
            const pomTopY = gripTop - pomH, pomBotY = gripTop;
            const neckHW = gripHW;
            const cBez = (ax, ay, c1x, c1y, c2x, c2y, bx, by, t) => {
              const u = 1 - t;
              return { x: u*u*u*ax + 3*u*u*t*c1x + 3*u*t*t*c2x + t*t*t*bx, y: u*u*u*ay + 3*u*u*t*c1y + 3*u*t*t*c2y + t*t*t*by };
            };
            const N = 5; // fewer samples (vs 12)
            const pomOutline = [];
            const bulgeY = pomBotY - pomH * 0.5;
            pomOutline.push({ x: -neckHW, y: pomBotY });
            pomOutline.push({ x: neckHW, y: pomBotY });
            for (let i = 1; i <= N; i++) pomOutline.push(cBez(neckHW, pomBotY, neckHW, pomBotY - pomH * 0.15, pomHW, pomBotY - pomH * 0.25, pomHW, bulgeY, i / N));
            for (let i = 1; i <= N; i++) pomOutline.push(cBez(pomHW, bulgeY, pomHW, bulgeY - pomH * 0.2, pomHW * 0.7, pomTopY - pomH * 0.02, 0, pomTopY, i / N));
            for (let i = 1; i <= N; i++) pomOutline.push(cBez(0, pomTopY, -pomHW * 0.7, pomTopY - pomH * 0.02, -pomHW, bulgeY - pomH * 0.2, -pomHW, bulgeY, i / N));
            for (let i = 1; i <= N; i++) pomOutline.push(cBez(-pomHW, bulgeY, -pomHW, pomBotY - pomH * 0.25, -neckHW, pomBotY - pomH * 0.15, -neckHW, pomBotY, i / N));

            const pomF = pomOutline.map(p => rotY(p.x, p.y, pomDepth, cosA, sinA));
            const pomB = pomOutline.map(p => rotY(p.x, p.y, -pomDepth, cosA, sinA));

            if (backVis) {
              ctx.beginPath(); ctx.moveTo(pomB[0].x, pomB[0].y);
              for (let i = 1; i < pomB.length; i++) ctx.lineTo(pomB[i].x, pomB[i].y);
              ctx.closePath(); ctx.fillStyle = pomLoCol; ctx.fill();
            }
            for (let i = 0; i < pomOutline.length; i++) {
              const j = (i + 1) % pomOutline.length;
              fillPoly([pomF[i], pomF[j], pomB[j], pomB[i]], guardSideCol);
            ...
            if (frontVis) {
              ctx.beginPath(); ctx.moveTo(pomF[0].x, pomF[0].y);
              for (let i = 1; i < pomF.length; i++) ctx.lineTo(pomF[i].x, pomF[i].y);
              ctx.closePath(); ctx.fillStyle = pomCol; ctx.fill();
            }
          }
        ...
        // === LEFT SWORD — full detail, rotating 3D ===
        drawWeapon(W * 0.25, H * 0.65, H * 0.5, H * 0.5 * 0.025, 0, {
          type: weaponTypes[0], zRot: time * 1.5, irr: {},
          wx: 0, wy: 0, wz: 3, darkness: 0
        });

        // === RIGHT SWORD — lightweight version ===
        drawWeaponLite(W * 0.75, H * 0.65, H * 0.5, H * 0.5 * 0.025, 0, {
          type: weaponTypes[0], zRot: time * 1.5, irr: {},
          wx: 0, wy: 0, wz: 3, darkness: 0
        });
