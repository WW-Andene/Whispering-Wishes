  ...
  return (
    ...
    <div className={`min-h-screen ${visualSettings.oledMode ? 'oled-mode' : ''} ${visualSettings.animationsEnabled === 'off' ? 'no-animations' : ''} ${visualSettings.animationsEnabled === 'full' ? 'animations-full' : ''}`}>
            ...
            <div className="header-controls flex items-center gap-2">
              <button onClick={() => setActiveTab('profile')} aria-label={t('app.profile')} title={t('app.profile')} className="relative w-[48px] h-[48px] flex items-center justify-center overflow-hidden transition-all" style={activeTheme && activeTab === 'profile' ? { background: `${themeAccent}30`, borderRadius: '12px' } : undefined}>
                {state.profile.profilePic && collectionImages[state.profile.profilePic]
                  ...
                  : <img src="./navicon/Icon_Setting.png" alt="" className={`w-6 h-6 ${activeTab === 'profile' ? 'opacity-100' : 'opacity-60'}`} />
                }
              </button>
            </div>
          </div>
      ...
      <nav ref={tabNavRef} className="kuro-card fixed bottom-3 left-3 right-3 z-50 flex items-center justify-center overflow-x-auto scrollbar-hide" style={{ position: 'fixed', zIndex: 50, height: 60, borderRadius: 14, marginBottom: 'var(--safe-area-bottom, 0px)', overflow: 'hidden', ...(activeTheme ? { borderColor: `${themeAccent}30` } : {}) }} role="tablist" aria-label={t('app.mainNavigation')} onKeyDown={(e) => {
        ...
        <TabButton active={activeTab === 'tracker'} onClick={() => setActiveTab('tracker')} tabRef={tabNavRef} tabId="tracker" accentColor={themeAccent}><img src="./navicon/Icon_Convene.webp" alt="" className="w-4 h-4" /> {t('app.navTracker')}</TabButton>
        <TabButton active={activeTab === 'events'} onClick={() => setActiveTab('events')} tabRef={tabNavRef} tabId="events" accentColor={themeAccent}><img src="./navicon/Icon_Event.webp" alt="" className="w-4 h-4" /> {t('app.navEvents')}</TabButton>
        <TabButton active={activeTab === 'map'} onClick={() => setActiveTab('map')} tabRef={tabNavRef} tabId="map" accentColor={themeAccent}><img src="./navicon/Icon_Map.png" alt="" className="w-4 h-4" /> {t('app.navMap')}</TabButton>
        <TabButton active={activeTab === 'planner'} onClick={() => setActiveTab('planner')} tabRef={tabNavRef} tabId="planner" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-4 h-4" /> {t('app.navPlan')}</TabButton>
        ...
        <TabButton active={activeTab === 'teams'} onClick={() => setActiveTab('teams')} tabRef={tabNavRef} tabId="teams" accentColor={themeAccent}><img src="./navicon/Icon_Team.webp" alt="" className="w-4 h-4" /> {t('app.navTeams')}</TabButton>
        <TabButton active={activeTab === 'gathering'} onClick={() => setActiveTab('gathering')} tabRef={tabNavRef} tabId="gathering" accentColor={themeAccent}><img src="./navicon/Icon_Databank.webp" alt="" className="w-4 h-4" /> {t('app.navCollection')}</TabButton>
      </nav>
