  ...
  return (
    ...
    <div className={`min-h-screen ${visualSettings.oledMode ? 'oled-mode' : ''} ${visualSettings.animationsEnabled === 'off' ? 'no-animations' : ''} ${visualSettings.animationsEnabled === 'full' ? 'animations-full' : ''}`}>
      ...
      <nav ref={tabNavRef} className="kuro-card fixed bottom-3 left-3 right-3 z-50 flex items-center justify-center overflow-x-auto scrollbar-hide" style={{ position: 'fixed', zIndex: 50, height: 62, borderRadius: 14, marginBottom: 'var(--safe-area-bottom, 0px)', overflow: 'hidden', ...(activeTheme ? { borderColor: `${themeAccent}30` } : {}) }} role="tablist" aria-label={t('app.mainNavigation')} onKeyDown={(e) => {
        ...
        <TabButton active={activeTab === 'tracker'} onClick={() => setActiveTab('tracker')} tabRef={tabNavRef} tabId="tracker" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navTracker')}</TabButton>
        <TabButton active={activeTab === 'events'} onClick={() => setActiveTab('events')} tabRef={tabNavRef} tabId="events" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navEvents')}</TabButton>
        <TabButton active={activeTab === 'map'} onClick={() => setActiveTab('map')} tabRef={tabNavRef} tabId="map" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navMap')}</TabButton>
        <TabButton active={activeTab === 'planner'} onClick={() => setActiveTab('planner')} tabRef={tabNavRef} tabId="planner" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navPlan')}</TabButton>
        <TabButton active={activeTab === 'calculator'} onClick={() => setActiveTab('calculator')} tabRef={tabNavRef} tabId="calculator" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navCalc')}</TabButton>
        <TabButton active={activeTab === 'analytics'} onClick={() => setActiveTab('analytics')} tabRef={tabNavRef} tabId="analytics" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navStats')}</TabButton>
        <TabButton active={activeTab === 'teams'} onClick={() => setActiveTab('teams')} tabRef={tabNavRef} tabId="teams" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navTeams')}</TabButton>
        <TabButton active={activeTab === 'gathering'} onClick={() => setActiveTab('gathering')} tabRef={tabNavRef} tabId="gathering" accentColor={themeAccent}><img src="./navicon/Icon_Guidebook.webp" alt="" className="w-6 h-6 object-contain" /> {t('app.navCollection')}</TabButton>
      </nav>
