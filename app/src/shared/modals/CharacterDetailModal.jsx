        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {data.tier && (
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold ${
                ...
                <span className="text-xs text-gray-400">ToA</span> {data.tier.toa}
                <span className="text-gray-600 mx-0.5">|</span>
                <span className="text-xs text-gray-400">WW</span> {data.tier.ww}
              </div>
            )}
            {data.region && (
          ...
          <div className="kuro-detail-box space-y-2">
            ...
            {data.debuffs?.length > 0 && (
              <div>
                <div className="text-xs text-gray-400 mb-1">Debuffs</div>
                <div className="flex flex-wrap gap-1">
            ...
            {data.dmgFocus?.length > 0 && (
              <div>
                <div className="text-xs text-gray-400 mb-1">Damage Focus</div>
                <div className="flex flex-wrap gap-1">
            ...
            {data.statScaling && (
              <div>
                <div className="text-xs text-gray-400 mb-1">Stat Scaling</div>
                <div className="flex flex-wrap gap-1">
            ...
            <div className="kuro-detail-box">
              ...
              <div className="space-y-1.5">
                {[1,2,3,4,5,6].map(s => {
                  ...
                  return (
                    <div key={s} className={`flex items-center gap-2 text-xs ${!unlocked ? 'opacity-50' : ''}`}>
          ...
          {CHAR_BUFF_TABLE[name]?.note && (
            <div className="kuro-detail-box">
              ...
              <p className="text-xs text-gray-300 leading-relaxed">{CHAR_BUFF_TABLE[name].note}</p>
            </div>
          )}
          ...
          <div className={`p-3 rounded-xl border ${hasWeapon ? colors.border : 'border-gray-700/50'} ${hasWeapon ? `bg-gradient-to-r ${colors.bg} from-transparent` : 'bg-white/[0.02]'}`} style={!hasWeapon ? { opacity: 0.55 } : undefined}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-bold ${hasWeapon ? 'text-yellow-400' : 'text-gray-500'}`}>{data.bestWeapon}</div>
                {weaponData && (
                  <>
                    <div className="text-gray-400 text-xs mt-0.5">{weaponData.type} • {weaponData.baseAtk ? `${weaponData.baseAtk} Base ATK` : ''}{weaponData.baseAtk && weaponData.stat ? ' • ' : ''}{weaponData.stat}{weaponData.subStatValue ? ` ${weaponData.subStatValue}` : ''}</div>
                    <div className="text-gray-400 text-xs mt-1 leading-relaxed">{weaponData.passive}</div>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-gray-400 text-xs">Main Echo</div>
                </div>
              </div>
              {data.bestEchoes[1] && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-gray-400 text-xs">Echo Set</div>
                </div>
              </div>
              )}
            </div>
                        ...
                        {members.map((member, j) => {
                          ...
                          return (
                            <div key={j} className={`flex flex-col items-center gap-1 flex-1 min-w-0 ${!memberOwned ? 'opacity-50' : ''}`}>
                              ...
                              <span className="text-xs text-gray-400 text-center leading-tight truncate w-full">{member}</span>
                            </div>
                          );
                        })}
                      </div>
          ...
          <div>
            <h3 className="text-white font-semibold text-base mb-2 flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500 font-normal ml-auto">Lv.1 ATK%</span>
            </h3>
                ...
                {SKILL_MULTIPLIERS[name].map(([type, skillName, mult], i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-start gap-1.5 px-2 py-1 rounded ${typeBg[type] || 'bg-white/5'}`}>
                      <span className={`text-xs font-medium w-14 shrink-0 pt-0.5 ${typeColors[type] || 'text-gray-400'}`}>{type}</span>
                      <span className="text-xs text-gray-200 font-medium min-w-0 shrink-0">{skillName}</span>
                      <span className="text-xs text-gray-400 ml-auto text-right pl-1">{mult}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              ...
              </> : <div className="text-gray-500 text-xs col-span-2">No ascension data</div>}
            </div>
          </div>
