        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {data.dmg > 0 && (
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-500/10 border border-red-500/30">
                <span className="text-base text-gray-400">DMG</span>
          ...
          {data.enemyRes && (
            <div className="p-3 rounded-xl border border-red-500/20" style={{ background: 'rgba(239,68,68,0.05)' }}>
              <div className="text-base text-red-400 uppercase tracking-wider mb-2 font-semibold">Elemental Resistance</div>
                  ...
                  <div key={setName} className="p-2 rounded-lg" style={{ background: `${setColor}10`, borderLeft: `3px solid ${setColor}80` }}>
                    ...
                    {setData ? (
                      <div className="space-y-0.5">
                        {setData.p2 && <div className="text-base text-gray-400"><span className="text-gray-500">2pc:</span> {setData.p2}</div>}
                        {setData.p3 && <div className="text-base text-gray-400"><span className="text-gray-500">3pc:</span> {setData.p3}</div>}
                        {setData.p5 && <div className="text-base text-gray-400"><span className="text-gray-500">5pc:</span> {setData.p5}</div>}
                      </div>
                    ) : (
                      <div className="text-base text-gray-500 italic">Set data not available</div>
                    )}
                  </div>
                );
              })}
                ...
                {usedBy.map(charName => {
                  ...
                  return (
                    <div key={charName} className={`flex flex-col items-center gap-1 ${!owned ? 'opacity-50' : ''}`}>
                      ...
                      <span className="text-base text-gray-400 text-center leading-tight max-w-[56px] truncate">{charName}</span>
                    </div>
                  );
                })}
              </div>
