...
const BannerCard = memo(({ item, type, stats, bannerImage, visualSettings, endDate, timerColor }) => {
  ...
  return (
    ...
    <div className="relative overflow-hidden rounded-xl border banner-card" style={{ minHeight: 'var(--height-banner-tracker)', isolation: 'isolate', borderColor: style.borderColor, boxShadow: isFull ? 'none' : BANNER_SUBTLE_SHADOW }}>
      {imgUrl && (
        <div
          ...
          <div className="flex gap-2 flex-wrap">
            {(item.featured4Stars || []).map(n => (
              <div key={n} className="inline-flex flex-col items-center gap-0.5">
                ...
                <span className="block w-12 text-[12px] text-cyan-300 bg-cyan-500/30 px-1.5 py-0.5 rounded backdrop-blur-sm text-center truncate" title={n}>{n}</span>
              </div>
            ))}
          </div>
        </div>
