...
const TabButton = memo(({ active, onClick, children, tabRef, tabId, accentColor }) => {
  ...
  return (
    <button
      ...
      className={`relative flex flex-col items-center justify-center gap-0.5 px-2.5 py-1 text-xs font-medium transition-all duration-300 whitespace-nowrap group active:scale-[0.97] ${active && !accent ? 'text-yellow-400' : !active ? 'text-gray-500 hover:text-gray-300' : ''}`}
