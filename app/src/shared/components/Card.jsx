...
const TabButton = memo(({ active, onClick, children, tabRef, tabId, accentColor }) => {
  ...
  return (
    ...
    >
      <div className={`relative z-10 p-1 transition-all duration-300 ${active && !accent ? 'bg-yellow-500/10 shadow-lg shadow-yellow-500/25' : !active ? 'group-hover:bg-white/5 group-hover:shadow-md group-hover:shadow-white/5' : ''}`} style={{ borderRadius: '8px', ...(active ? { filter: `drop-shadow(0 0 5px ${accent ? accent + '80' : 'rgba(237,175,24,0.5)'}) drop-shadow(0 0 2px rgba(0,0,0,0.35))`, ...(accent ? { background: accent + '1a', boxShadow: `0 10px 15px -3px ${accent}40` } : {}) } : { filter: 'drop-shadow(0 0 2px rgba(0,0,0,0.35))' }) }}>
        {icon}
      </div>
