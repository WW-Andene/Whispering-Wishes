        ...
        {toasts.map(toast => (
          <div key={toast.id} className={`kuro-toast px-4 py-3 rounded-lg flex items-center gap-2 text-base font-medium pointer-events-auto border ${toast.type === 'warning' ? 'text-amber-900 border-amber-300/40' : 'text-white border-white/20'}`} style={{
            ...
            {toast.onUndo && (
              ...
              }} className={`ml-2 px-2 py-0.5 rounded text-sm font-bold uppercase tracking-wider transition-colors flex-shrink-0 ${toast.type === 'warning' ? 'bg-amber-900/15 hover:bg-amber-900/25 text-amber-900' : 'bg-white/20 hover:bg-white/30 text-white'}`}>
                Undo
              </button>
            )}
          </div>
