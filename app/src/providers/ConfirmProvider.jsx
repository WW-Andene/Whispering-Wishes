          ...
          <div className="kuro-card w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
            <div className="kuro-card-inner rounded-2xl overflow-hidden">
              <div className="p-5 text-center">
                <h3 className="text-white font-semibold text-base mb-2">{state.title}</h3>
                <p className="text-gray-400 text-sm leading-relaxed whitespace-pre-line">{state.message}</p>
              </div>
              <div className="border-t border-[var(--border-medium)] flex">
                ...
                <button onClick={() => handleClose(true)} className={`flex-1 py-3.5 text-base font-semibold active:bg-white/5 transition-colors ${state.destructive ? 'text-red-400' : 'text-cyan-400'}`}>{state.confirmLabel}</button>
              </div>
            </div>
          </div>
        )}
