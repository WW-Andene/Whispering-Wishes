          ...
          <div className="kuro-card w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
            <div className="kuro-card-inner rounded-2xl overflow-hidden">
              ...
              <div className="border-t border-[var(--border-medium)] flex">
                ...
                <button onClick={() => handleClose(true)} className={`flex-1 py-3.5 text-xl font-semibold active:bg-white/5 transition-colors ${state.destructive ? 'text-red-400' : 'text-cyan-400'}`}>{state.confirmLabel}</button>
              </div>
            </div>
          </div>
        )}
