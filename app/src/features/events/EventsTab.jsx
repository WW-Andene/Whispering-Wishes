...
function EventsTab({
  ...
  return (
    ...
    <div className="kuro-calc space-y-3 tab-content">
        ...
        <CardHeader action={
          <div className="flex items-center gap-2">
            <button
              ...
              className={`text-xs flex items-center gap-1 transition-colors p-1.5 min-h-[44px] min-w-[44px] justify-center rounded-lg ${refreshCooling ? 'text-gray-600 cursor-not-allowed' : 'text-cyan-400 hover:text-cyan-300 hover:bg-white/5'}`}
            >
              <RefreshCcw size={12} className={refreshCooling ? 'animate-spin' : ''} /> Refresh Timers
            </button>
            <span className="text-gray-400 text-xs">Server: {state.server}</span>
          </div>
        ...
        <CardBody className="space-y-2">
              <div className="p-2.5 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                ...
                <div className="mt-1.5 flex items-center gap-2">
                  ...
                  <span className="text-gray-400 text-xs flex-shrink-0">{progressStats.doneCount}/{progressStats.totalCount} done</span>
                </div>
              </div>
            ...
            {expired.length > 0 && (
              <>
                <div className="flex items-center gap-2 pt-2 pb-1">
                  ...
                  <span className="text-gray-500 text-xs font-medium uppercase tracking-wider">Expired</span>
                  <div className="flex-1 h-px bg-gray-700/50" />
                </div>
      ...
      <p className="text-gray-500 text-xs text-center content-layer sticky bottom-0 py-2 kuro-gradient-fade-up">Reset times based on {state.server} server (UTC{getServerOffset(state.server) >= 0 ? '+' : ''}{getServerOffset(state.server)})</p>
    </div>
    </TabErrorBoundary>
    </div>
  );
