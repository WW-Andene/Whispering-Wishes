    ...
    <Card id="team-dps-comparison">
      <CardHeader action={
        <button onClick={async () => { if (await confirm?.({ title: 'Clear comparison', message: 'Remove all comparison entries?', confirmLabel: 'Clear', destructive: true })) { setTeamCompareEntries([]); haptic.light(); } }}
          className="kuro-btn text-base" aria-label="Clear all team comparisons">
          Clear All
        </button>
      ...
      <CardBody>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3 p-2 rounded-lg border border-[var(--border-medium)]" style={{ background: 'var(--bg-stat)' }}>
          ...
          <span className="text-gray-400 text-base font-medium">Target:</span>
          <button onClick={() => { setEnemyEchoSearch(''); setEnemyEchoModalOpen(true); haptic.light(); }}
            className="kuro-btn text-base px-2 py-1 flex-1 min-w-[120px] max-w-[240px] text-left truncate">
            {enemyEcho ? (() => {
          ...
          <div className="flex items-center gap-1">
            <span className="text-gray-500 text-base">Lv.</span>
            <input type="text" inputMode="numeric" value={enemyLevel}
              ...
              className="kuro-input w-12 text-base px-1 py-0.5 text-center" />
          </div>
          <span className="text-gray-600 text-base">DEF {792 + 8 * (Number(enemyLevel) || 90)}</span>
        </div>
            ...
            return (
              <div key={entry.id} className="group p-2.5 rounded-lg border border-[var(--border-medium)] relative" style={{ background: 'var(--bg-stat)' }}>
                <div className="flex items-center justify-between mb-1.5 pr-8">
                  <span className="text-base font-medium text-gray-300 truncate" title={entry.slots.filter(Boolean).join(' / ')}>
                  ...
                  {s.members.map((m, mi) => {
                    ...
                    return (
                      <div key={mi} className={`flex-1 min-w-0 p-1.5 rounded-lg border text-center ${rarity5 ? 'border-yellow-500/50 glow-gold bg-yellow-500/10' : 'border-purple-500/50 glow-purple bg-purple-500/10'}`}>
                        <div className="text-base font-semibold truncate" style={{ color: getElementColor(m.d.element), textShadow: `0 0 8px ${getElementColor(m.d.element)}60` }}>{m.name}</div>
                        <div className={`text-base ${rarity5 ? 'text-yellow-400' : 'text-purple-400'}`}>{rarity5 ? '★★★★★' : '★★★★'}</div>
                        <span className={`text-base px-1 py-0.5 rounded ${rc2.bg} ${rc2.text} inline-block mt-0.5`}>{m.d.role}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                {[
                  ...
                  <div key={bi} className={bi < 2 ? 'mb-1' : 'mb-0.5'}>
                    <div className="flex items-baseline justify-between mb-0.5">
                      <span className="text-gray-400 text-base">{bar.label}</span>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 pt-1.5 border-t border-[var(--border-medium)]">
                  <div className="text-base"><span className="text-gray-500">DPS: </span><span className="text-white font-medium">{s.mainDps.name}</span></div>
                  <div className="text-base"><span className="text-gray-500">{s.mainDps.scaling !== 'ATK' ? s.mainDps.scaling : 'ATK'}: </span><span className="text-yellow-400 kuro-number">{s.effAtk?.toLocaleString('en-US')}</span></div>
                  <div className="text-base"><span className="text-gray-500">CR: </span><span className="text-cyan-400 kuro-number">{s.critRate.toFixed(0)}%</span></div>
                  <div className="text-base"><span className="text-gray-500">CD: </span><span className="text-cyan-400 kuro-number">{s.critDmg.toFixed(0)}%</span></div>
                  <div className="text-base"><span className="text-gray-500">Rot: </span><span className="text-gray-300 kuro-number">{s.mainDps.d.rotTime || 25}s</span></div>
                  {s.mainDps.scaling !== 'ATK' && <div className="text-base"><span className="text-violet-400">{s.mainDps.scaling} scaling</span></div>}
                  {s.defShred > 0 && <div className="text-base"><span className="text-gray-500">DEF↓ </span><span className="text-red-400 kuro-number">{Math.round(s.defShred)}%</span></div>}
                </div>
              </div>
            );
          })}
        ...
        {teamCompareEntries.length < 5 && (
          <p className="text-gray-500 text-base text-center mt-2">Tap <span className="text-yellow-400">+ Compare</span> to add more ({5 - teamCompareEntries.length} left)</p>
        )}
      </CardBody>
    </Card>
  );
