...
function PityHistogram({ statsTabData }) {
  ...
  return (
    <Card>
      <CardHeader action={<span className="text-gray-500 text-xs">{fiveStars.length} Convenes</span>}>
        <span className="flex items-center gap-1.5"><BarChart3 size={14} /> 5★ Pity Distribution</span>
      </CardHeader>
      <CardBody>
            ...
            return (
              <div key={label} className="flex-1 flex flex-col items-center" title={`${label} pity: ${count} Convene${count !== 1 ? 's' : ''}`}>
                <div className="w-full relative" style={{ height: '72px' }}>
                  {count > 0 && (
                    <div
                      className="absolute left-0 right-0 text-xs text-center font-bold kuro-number"
                      style={{
                        bottom: `${height}%`,
                        marginBottom: '4px',
                        color: color,
        ...
        <div className="flex gap-1.5">
          {allBuckets.map(label => (
            <div key={label} className="flex-1 text-xs text-gray-400 text-center">{label}</div>
          ))}
        </div>

        <div className="mt-3 pt-3 border-t border-[var(--border-medium)] grid grid-cols-3 gap-2 text-center">
          <div>
            <div className="text-emerald-400 font-bold text-sm kuro-number kuro-tshadow-glow-emerald">{minPity}</div>
            <div className="text-gray-400 text-xs">Lowest</div>
          </div>
          <div>
            <div className="text-yellow-400 font-bold text-sm kuro-number kuro-tshadow-glow-gold">{avgPity}</div>
            <div className="text-gray-400 text-xs">Average</div>
          </div>
          <div>
            <div className="text-red-400 font-bold text-sm kuro-number kuro-tshadow-glow-red">{maxPity}</div>
