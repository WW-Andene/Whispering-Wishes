  ...
  return createPortal(
    ...
    >
      <div className="sticky top-0 bg-cyan-900/40 border-b border-cyan-500/30 p-2.5 flex items-center justify-between">
        <span className="text-cyan-300 text-base font-bold flex items-center gap-1.5"><Settings size={14} /> Visual Settings</span>
        <div className="flex gap-1">
          <div className="flex gap-0.5 mr-1">
            <button onClick={() => saveMiniPanelPosition('top-left')} aria-label="Move to top-left" className={`w-5 h-5 rounded text-base ${miniPanelPosition === 'top-left' ? 'bg-cyan-500 text-black' : 'bg-white/10 text-gray-400'}`}>↖</button>
            <button onClick={() => saveMiniPanelPosition('top-right')} aria-label="Move to top-right" className={`w-5 h-5 rounded text-base ${miniPanelPosition === 'top-right' ? 'bg-cyan-500 text-black' : 'bg-white/10 text-gray-400'}`}>↗</button>
            <button onClick={() => saveMiniPanelPosition('bottom-left')} aria-label="Move to bottom-left" className={`w-5 h-5 rounded text-base ${miniPanelPosition === 'bottom-left' ? 'bg-cyan-500 text-black' : 'bg-white/10 text-gray-400'}`}>↙</button>
            <button onClick={() => saveMiniPanelPosition('bottom-right')} aria-label="Move to bottom-right" className={`w-5 h-5 rounded text-base ${miniPanelPosition === 'bottom-right' ? 'bg-cyan-500 text-black' : 'bg-white/10 text-gray-400'}`}>↘</button>
          </div>
      ...
      <div className="p-3 space-y-3">
        ...
        <button
          ...
          className={`w-full py-2 rounded text-base font-medium border transition-all ${framingMode ? 'bg-emerald-500/30 text-emerald-400 border-emerald-500/50' : 'bg-white/5 text-gray-400 border-[var(--border-medium)] hover:bg-white/10'}`}
        >
        ...
        {framingMode && editingImage && (
          <div className="p-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
            <div className="text-emerald-400 text-base font-medium mb-2 truncate">
            ...
            <div className="grid grid-cols-3 gap-1 mb-2">
              ...
              <button onClick={resetEditingFraming} className="p-2 bg-red-500/20 hover:bg-red-500/30 rounded text-red-400 text-base" aria-label="Reset framing">Reset</button>
            ...
            <div className="text-center text-gray-500 text-base mt-2">Tap another image to edit it</div>
          </div>
        )}

        {framingMode && !editingImage && (
          <div className="p-2 bg-white/5 border border-[var(--border-medium)] rounded-lg text-center">
            <div className="text-gray-400 text-base">Tap any character image to frame it (Collection, Teams, or Detail modal)</div>
          </div>
        )}
        ...
        {framingMode && detailModal.show && detailModal.type === 'character' && (() => {
          ...
          return (
            <div className="p-2 bg-orange-500/10 border border-orange-500/30 rounded-lg">
              <div className="text-orange-400 text-base font-medium mb-2 truncate">
              ...
              <div className="grid grid-cols-3 gap-1 mb-2">
                ...
                <button onClick={() => saveImageFraming(infoKey, { x: 0, y: 0, zoom: 100 })} className="p-2 bg-red-500/20 hover:bg-red-500/30 rounded text-red-400 text-base" aria-label="Reset info framing">Reset</button>
              ...
              <div className="text-center text-gray-500 text-base mt-2">Adjusts the character info panel header image</div>
            </div>
          );
        })()}
        ...
        {framingMode && Object.keys(imageFraming).length > 0 && (
          <button
            ...
            className="w-full py-2 rounded text-base font-medium border transition-all bg-cyan-500/10 text-cyan-400 border-cyan-500/30 hover:bg-cyan-500/20"
          >
        ...
        <button
          ...
          className={`w-full py-2 rounded text-base font-medium border transition-all ${bgFramingMode ? 'bg-cyan-500/30 text-cyan-400 border-cyan-500/50' : 'bg-white/5 text-gray-400 border-[var(--border-medium)] hover:bg-white/10'}`}
        >
        ...
        {bgFramingMode && editingBgTarget && (
          <div className="p-2 bg-cyan-500/10 border border-cyan-500/30 rounded-lg">
            <div className="text-cyan-400 text-base font-medium mb-2">
              Editing: {editingBgTarget === 'header' ? 'Header' : editingBgTarget === 'nav' ? 'Navigation' : 'Background'}
            </div>
            <div className="text-gray-400 text-base mb-2 font-mono text-center">{getBgPositionLabel()}</div>
            <div className="grid grid-cols-3 gap-1 w-24 mx-auto mb-2">
              <div />
              <button onClick={() => updateBgPosition(0, -2)} className="bg-white/10 text-white rounded p-1 text-base hover:bg-white/20 active:scale-95">▲</button>
              <div />
              <button onClick={() => updateBgPosition(-2, 0)} className="bg-white/10 text-white rounded p-1 text-base hover:bg-white/20 active:scale-95">◀</button>
              <button onClick={() => {
              ...
              }} className="bg-red-500/20 text-red-400 rounded p-1 text-base hover:bg-red-500/30 active:scale-95">RST</button>
              <button onClick={() => updateBgPosition(2, 0)} className="bg-white/10 text-white rounded p-1 text-base hover:bg-white/20 active:scale-95">▶</button>
              <div />
              <button onClick={() => updateBgPosition(0, 2)} className="bg-white/10 text-white rounded p-1 text-base hover:bg-white/20 active:scale-95">▼</button>
              <div />
            </div>
            <p className="text-gray-500 text-base text-center">Tap header, nav, or background to switch</p>
          </div>
        )}
        ...
        {!framingMode && !bgFramingMode && (
          <>
            <button
              ...
              className="w-full py-1.5 rounded text-base bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 hover:bg-yellow-500/30"
            >
