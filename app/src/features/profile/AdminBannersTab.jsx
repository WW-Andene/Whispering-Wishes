    ...
    <>
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <input type="text" placeholder="Version (e.g., 3.1)" value={bannerForm.version} onChange={(e) => updateBannerForm('version', e.target.value)} className="kuro-input text-sm py-1" aria-label="Banner version" />
          <input type="number" placeholder="e.g. 1" value={bannerForm.phase} onChange={(e) => updateBannerForm('phase', e.target.value)} className="kuro-input text-sm py-1" aria-label="Banner phase" />
          <input type="datetime-local" placeholder="Start Date" value={bannerForm.startDate} onChange={(e) => updateBannerForm('startDate', e.target.value)} className="kuro-input text-sm py-1" aria-label="Banner start date" />
          <input type="datetime-local" placeholder="End Date" value={bannerForm.endDate} onChange={(e) => updateBannerForm('endDate', e.target.value)} className="kuro-input text-sm py-1" aria-label="Banner end date" />
        </div>
      </div>

      <div className="space-y-2">
        <h3 className="text-white text-md font-medium">Featured Resonators (JSON)</h3>
        <textarea className="kuro-input w-full h-32 text-sm font-mono" value={bannerForm.charsJson} onChange={(e) => updateBannerForm('charsJson', e.target.value)} placeholder="Paste characters array JSON" aria-label="Featured resonators JSON" />
      </div>

      <div className="space-y-2">
        <h3 className="text-white text-md font-medium">Featured Weapons (JSON)</h3>
        <textarea className="kuro-input w-full h-32 text-sm font-mono" value={bannerForm.weapsJson} onChange={(e) => updateBannerForm('weapsJson', e.target.value)} placeholder="Paste weapons array JSON" aria-label="Featured weapons JSON" />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-gray-300 text-sm w-28">Tidal Chorus</span>
            <input type="text" placeholder="https://i.ibb.co/..." value={bannerForm.standardCharImg} onChange={(e) => updateBannerForm('standardCharImg', e.target.value)} className="kuro-input flex-1 text-sm py-1" aria-label="Tidal Chorus banner image URL" />
          </div>
          <div className="flex items-center gap-2">
            <span className="text-gray-300 text-sm w-28">Winter Brume</span>
            <input type="text" placeholder="https://i.ibb.co/..." value={bannerForm.standardWeapImg} onChange={(e) => updateBannerForm('standardWeapImg', e.target.value)} className="kuro-input flex-1 text-sm py-1" aria-label="Winter Brume banner image URL" />
          </div>
        </div>
      </div>
        ...
        <div className="space-y-1">
          {[
            ...
            <div key={key} className="flex items-center gap-2">
              <span className="text-gray-300 text-sm w-28">{label}</span>
              <input type="text" placeholder="https://i.ibb.co/..." value={bannerForm[key]} onChange={(e) => updateBannerForm(key, e.target.value)} className="kuro-input flex-1 text-sm py-1" aria-label={`${label} image URL`} />
            </div>
          ))}
        </div>
