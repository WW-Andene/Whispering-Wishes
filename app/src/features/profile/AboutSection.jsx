    ...
    <Card>
      ...
      <CardBody className="space-y-3 text-xs text-gray-400">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-xs">Version {APP_VERSION}</p>
        </div>

        <div className="text-center">
          <p className="text-gray-400 text-xs mb-1">Questions, issues, or feedback?</p>
          <a
        ...
        <p className="text-center text-xs text-gray-500 pt-1">© {currentYear} <span onClick={handleAdminTap} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleAdminTap(); } }} tabIndex={0} role="button" className="cursor-pointer select-none" style={adminTapCount >= 3 ? { color: 'rgba(237,175,24,0.5)', transition: 'color 0.3s' } : undefined}>{`Whispering Wishes Ver.${APP_VERSION}`}</span> by <a href="https://www.reddit.com/u/WW_Andene" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-400 transition-colors">u/WW_Andene</a> • Made with ♡ for the WuWa community.</p>
        ...
        {sections.map(({ key, label, content }) => (
          <div key={key} style={{ background: 'var(--bg-stat)', borderRadius: 'var(--radius-md)' }}>
            ...
            {aboutSections[key] && (
              <div className="text-xs text-gray-400" style={{ padding: '0 12px 10px' }}>
                {content}
              </div>
            )}
          </div>
