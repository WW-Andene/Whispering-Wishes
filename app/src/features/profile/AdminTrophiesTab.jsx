  ...
  return (
    <div className="space-y-4">
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
        ...
        <div className="mb-3">
          <div className="text-gray-400 text-xs font-medium mb-1 uppercase tracking-wider">Current Trophies ({trophies?.list?.length || 0})</div>
          <div className="max-h-[200px] overflow-y-auto kuro-scroll bg-black/30 rounded border border-[var(--border-medium)] p-2 space-y-0.5">
            {(trophies?.list || []).map(t => (
              <div key={t.id} className="flex items-center gap-2 py-0.5">
                <span className="text-xs font-mono text-gray-500 w-20 flex-shrink-0 truncate" title={t.id}>{t.id}</span>
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: t.color }} />
                <span className={`text-xs flex-1 truncate ${trophyOverrides[t.id] ? 'text-amber-400' : 'text-gray-300'}`} title={t.name}>{t.name}</span>
                <span className="text-xs text-gray-500 flex-shrink-0">{t.name.length}ch</span>
              </div>
            ))}
        ...
        <button
          ...
          className="w-full mb-3 px-3 py-1.5 bg-white/5 border border-[var(--border-medium)] text-gray-300 rounded text-xs hover:bg-white/10 transition-colors"
        >
          Export Current Trophies as JSON
        </button>

        {/* JSON import textarea */}
        <div className="text-gray-400 text-xs font-medium mb-1 uppercase tracking-wider">Import Overrides (JSON)</div>
        <textarea
          className="kuro-input w-full h-40 text-xs font-mono"
        ...
        />
        <p className="text-gray-400 text-xs mt-1 mb-2">Only include trophies you want to rename. Omit <code className="text-amber-400/60">desc</code> to keep the original description.</p>
        ...
        {Object.keys(trophyOverrides).length > 0 && (
          <div className="mt-3 bg-amber-500/10 border border-amber-500/20 rounded p-2 text-xs text-amber-400">
