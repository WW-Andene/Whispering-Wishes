    ...
    <div className="space-y-4">
      ...
      {activePlayersHistory.length > 1 && (
        <div className="bg-white/5 border border-[var(--border-medium)] rounded-lg p-3">
          <div className="text-gray-400 text-base font-medium mb-2 uppercase tracking-wider">Session Activity</div>
          <div className="h-24">
            {(() => {
      ...
      <div className="bg-white/5 border border-[var(--border-medium)] rounded-lg p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="text-gray-400 text-base font-medium uppercase tracking-wider">Registered Players</div>
          <div className="text-gray-400 text-base">{adminPlayerList ? adminPlayerList.length : '—'} total</div>
        </div>
        {!adminPlayerList ? (
              ...
              <div key={p.firebaseKey} className="flex items-center justify-between p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-400 text-base w-4 text-right flex-shrink-0">{i + 1}</span>
                    <span className="text-white text-base font-mono font-medium truncate">{p.uid || p.id}</span>
                    {p.uid && p.id !== p.uid && (
                      <span className="text-gray-500 text-base font-mono flex-shrink-0">({p.id.slice(0, 6)}…)</span>
                    )}
                  </div>
                  <div className="flex items-center gap-3 ml-6 mt-0.5">
                    <span className="text-gray-400 text-base">Avg: <span className="text-yellow-400">{typeof p.avgPity === 'number' ? p.avgPity.toFixed(1) : p.avgPity}</span></span>
                    <span className="text-gray-400 text-base">5★: <span className="text-purple-400">{p.fiveStars}</span></span>
                    <span className="text-gray-400 text-base">Convenes: <span className="text-gray-300">{p.totalPulls}</span></span>
                    <span className="text-gray-400 text-base">50/50: <span className="text-emerald-400">{p.won5050}W</span>/<span className="text-red-400">{p.lost5050}L</span></span>
                  </div>
                </div>
                <div className="text-gray-500 text-base text-right flex-shrink-0 ml-2">
      ...
      {/* Privacy Notice */}
      <div className="bg-white/5 border border-[var(--border-medium)] rounded-lg p-3 text-base text-gray-400 space-y-1">
      ...
      {/* Error Display */}
      {presenceError && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-base text-red-400 space-y-1.5">
          <div className="font-medium">⚠ Presence Error</div>
          <p>{presenceError}</p>
          <div className="text-red-400/70 text-base space-y-0.5">
            <p className="font-medium">Fix: Add this Firebase rule:</p>
            <pre className="bg-black/30 rounded p-2 text-base overflow-x-auto font-mono whitespace-pre">
{`"presence": {
  ".read": true,
  ".write": true
}`}
