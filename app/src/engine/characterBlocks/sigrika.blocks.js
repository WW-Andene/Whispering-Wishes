...
export const SIGRIKA_BLOCKS = [
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('117.67%+205.92%+264.75%'), category: 'heavyDmg' },
    note: 'Consumes 2 Runes of the same type for Runic Chain Whip (Stagnates nearby targets, not modeled). Sourced from the single "Runic Outburst" row (no per-variant breakdown published).',
  },
  {
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('117.67%+205.92%+264.75%'), category: 'heavyDmg' },
    note: 'Consumes 2 different-type Runes for Runic Outburst (pure bonus DMG, no extra effect).',
  },
  {
  ...
  },
  {
    ...
    note: "Where Trust Leads Me!'s own DMG Multiplier +30% (Liberation-type node, counted as Echo Skill DMG on the actual hit — kept as libDmg per this file's convention for a Liberation-type move's own multiplier, matching the same pattern as Roccia's S5) — cast-scoped (instant, no persistent duration).",
  },
  {
    id: 'sigrika.chain.s6',
    source: SOURCE, kind: 'buff',
    ...
    note: 'Flat value used as-is — no further scope detail sourced beyond the flat value, flagged as unverified. Kept passive.',
  },
];
