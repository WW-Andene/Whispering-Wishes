...
// [CHARACTER · LUMI] Lumi's TriggerBlock set — Layer 4 of the engine rewrite,
// migrated onto the single canonical schema (block.schema.js). Sourced from
// characters.js's already-audited CHAR_BUFF_TABLE['Lumi'], RESONANCE_CHAIN_DATA
// ['Lumi'] (+ its own detailed 2026-09-01 re-audit comment, read directly for each
// node's real mechanic), SKILL_MULTIPLIERS['Lumi'], and CHARACTER_ROTATIONS['Lumi'].
// No new numbers invented. S1 correctly has NO block — pure STA-restore utility with
// zero DPS component, per the audit's own zeroing. Laser (S5's real scope) has no
...
export const LUMI_BLOCKS = [
  ...
  //    being cast from the Resonance Skill/Forte slot) ──
  {
    ...
    damage: { hits: parseSkillMultiplierHits('56.33%×3'), category: 'skillDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('183.31%×2'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('120.25% → 138.32%+27.67%×5 → 93.73%+218.69%'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('251.70%'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('31.81%×3'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    damage: { hits: Array.from({ length: 6 }, () => ({ atkPct: 81.52 })), category: 'basicDmg', basis: 'ATK' },
