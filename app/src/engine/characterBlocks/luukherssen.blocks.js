...
// [CHARACTER · LUUK HERSSEN] Luuk Herssen's TriggerBlock set — Layer 4 of the engine
// rewrite, migrated onto the single canonical schema (block.schema.js). Sourced from
// characters.js's already-audited CHAR_BUFF_TABLE['Luuk Herssen'] (all zero, plus its
// own Tune Break sub-object — no DPS-representable buffs), RESONANCE_CHAIN_DATA
// ['Luuk Herssen'] (+ its own audit comment, read directly for each node's real
// mechanic), SKILL_MULTIPLIERS['Luuk Herssen'], and CHARACTER_ROTATIONS
// ['Luuk Herssen']. No new numbers invented. S1/S3/S5's approximated
// totalMult/basicDmg values are kept as documented approximations per the source
// table's own reasoning, not re-derived further.
//
...
export const LUUK_HERSSEN_BLOCKS = [
  // ── Damage blocks (from SKILL_MULTIPLIERS) ──
  {
    ...
    damage: { hits: parseSkillMultiplierHits('72.67%×3'), basis: 'ATK' },
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('155.47%'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    damage: { hits: parseSkillMultiplierHits('306.90%'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    damage: { hits: parseSkillMultiplierHits('745.54% + 49.71%×5'), category: 'basicDmg', basis: 'ATK' },
  ...
  },
  {
    ...
    timing: {}, target: { scope: 'self' }, effects: [],
    damage: { hits: parseSkillMultiplierHits('500%'), basis: 'ATK' },
  ...
  // ── Base-kit Inherent Skill buff (Uncaused Diagnosis) ──
  {
    ...
    timing: { duration: 20 },
    target: { scope: 'self' },
    effects: [{ stat: 'atkPct', value: 25, stacking: 'refresh', source: 'teammate-ally-action' }],
  ...
  },
  {
    ...
    trigger: { type: 'passive' },
    timing: {}, target: { scope: 'self' },
    effects: [{ stat: 'basicDmg', value: 60, scopedToBlockId: "luukherssen.liberation.rewritten-in-winters-margins", source: 'self-kit' }],
