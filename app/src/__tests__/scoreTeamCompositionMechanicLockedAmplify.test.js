...
//
// Root cause: Phoebe's Confession-mode outro grants `amplify +100% "Spectro Frazzle DMG Amp
...
//
// Fixed by adding a small, explicit, data-driven MECHANIC_DAMAGE_APPLIERS allow-list: an amplify/
...
describe('universalStatApplies — mechanic-locked amplify buffs require a confirmed applier', () => {
...
describe('scoreTeamComposition — Phoebe no longer outranks Jinhsi\'s own curated real partner', () => {
  it("Phoebe's Frazzle amplify scores far lower for Jinhsi (no Frazzle synergy) than Shorekeeper", () => {
  ...
  it('Phoebe still scores her full Frazzle-amp value for Zani, her real Frazzle-DPS partner', () => {
    const withAmplify = scoreTeamComposition(['Zani', 'Zhezhi', 'Phoebe'], undefined, 'Zani').score;
    const saved = CHAR_BUFF_TABLE['Phoebe'].outroBuffs;
    CHAR_BUFF_TABLE['Phoebe'].outroBuffs = saved.filter(b => b.stat !== 'amplify');
    const withoutAmplify = scoreTeamComposition(['Zani', 'Zhezhi', 'Phoebe'], undefined, 'Zani').score;
    CHAR_BUFF_TABLE['Phoebe'].outroBuffs = saved;
    expect(withAmplify).toBeGreaterThan(withoutAmplify);
  });
});
