...
| ~~**10**~~ | ~~**Border opacity tokens**~~ ✅ DONE — Added 5 border tokens (`--border-subtle/default/medium/hover/bright`) to `:root`. Replaced 9 hardcoded border values across card, button, input, stat, header, corner decorations, divider, and pity ring track. | D-TOKEN-1 | `appcore-providers.jsx` | ✅ |
