...
Next: move to gap category (2) — simple additive schema fields (#8 flat damage, #4 stack-cap-increase,
#5 frequency/tick-rate, #16 missing damage categories, #17 per-hit basis split), per the agreed priority
order (mechanism backlog first, then simple fields, then new condition/trigger types, then complex calc
shapes, then structurally novel simulation mechanics last).
