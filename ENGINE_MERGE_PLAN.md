...
| Ciaccona | **Partial — 1 targeted gap-fix 2026-09-02 against a fresh user-provided Prydwen dump, NOT a full 8-dimension Phase A pass** | Not audited block-by-block against all 8 dimensions. Full comparison of `SKILL_MULTIPLIERS`/`CHAR_BUFF_TABLE`/`CHARACTER_ROTATIONS`/`desc` against the dump found everything else already exactly matching (including her already-correct 2026-09-01 `RESONANCE_CHAIN_DATA` re-audit for S1/S2/S4/S5, and S3 correctly left as a pure resource-grant with no block). **1 real fix**: S6 ("while in Solo Concert, deals 220% ATK Aero DMG, considered Liberation DMG") was correctly zeroed to `{}` in `RESONANCE_CHAIN_DATA` (a flat %-ATK proc doesn't fit that table's `{stat:value}` shape) but had never been built as its own damage block — same class of gap as Phrolova's Apparition of Beyond-Hecate, fixed the same session. Added `ciaccona.chain.s6`, sequence-6-gated, anchored to the same Basic ATK Stage 4 cast that starts Solo Concert (one representative pulse, same pattern already used for her own Symphonic Poem: Tonic block). 1 new test, full suite green (1293/1293). Her Erosion DOT migration (Phase 2) stays untouched/already-migrated from a prior session, not re-verified here. |
| *(remaining 50 characters)* | Not started | — |
