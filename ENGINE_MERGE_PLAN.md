...
| Qingxiao | **Audited 2026-09-02** | Clean — every damage block correctly `appliesTags:['shifting']` per the user-confirmed broad reading of "skills". `chain.s4`'s self-scoped-passive approximation (real mechanic needs a cross-character `ally-action` trigger) is a KNOWN, already-tracked gap (Engine development.md item 9's own Migration section) — not fixed here, since doing it correctly requires re-verifying S4's exact sourced trigger text (self-cast vs ally-cast) rather than guessing from memory, and getting it wrong would be worse than the documented approximation. Left as-is, not re-logged as new. |
