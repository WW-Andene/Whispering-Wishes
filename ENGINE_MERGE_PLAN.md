...
| Rebecca | **Partial — 4 real fixes 2026-09-02 against a fresh user-provided Prydwen dump, NOT a full 8-dimension Phase A pass; 1 value discrepancy flagged, not resolved** | Not audited block-by-block against all 8 dimensions. `RESONANCE_CHAIN_DATA`/`CHAR_BUFF_TABLE`/`CHARACTER_ROTATIONS` all already matched (including S4 correctly left unmodeled — buff-to-a-buff, no flat-schema equivalent — and S6's 900%-ATK bonus hit already correctly built as its own block from a prior session). **3 miscategorization fixes, the reverse of the usual pattern**: `rebecca.forte.rat-tat-tat-huntress` (was `heavyDmg`) and `rebecca.liberation.party-til-dawn` (was `libDmg`) both have kit text explicitly stating "considered Basic Attack DMG" despite their Heavy/Liberation input slots — fixed to `basicDmg`; `party-til-dawn`'s own note already said "auto-firing Basic ATK DMG" in prose, the code just never matched it, a bug hiding in plain sight. `rebecca.liberation.boom-fireworks` (also `libDmg`) has no explicit override text, but the dump's own damage-profile shows a literal 0 in the Liberation bucket despite the Ultimate obviously dealing real damage — inferred `basicDmg` too, same confidence tier as the "Skill Damage" row-label convention used on Lupa/Ciaccona. **`dmgFocus` fix**: was `['Basic ATK', 'Heavy ATK']` — her literal Heavy ATK category is 0 (everything routes to Basic per the above) — replaced with `['Basic ATK', 'Skill']`. **`SKILL_MULTIPLIERS` type-label fix**: the Rat-tat-tat!/Bang-bang-bang! row was typed `'Heavy ATK'` while `CHARACTER_ROTATIONS`'s own real step for it uses `type:'Forte'` — a silent lookup mismatch (`findSkillMultiplierRow()` requires exact type match even on its fuzzy fallback) independent of the category question, fixed to `'Forte'`, removed from the data-integrity baseline. **1 real value discrepancy found and deliberately NOT resolved**: `SKILL_MULTIPLIERS`'/`rebecca.blocks.js`'s Rat-tat-tat!/Bang-bang-bang!/Hack-Response-Meltdown values (already cross-verified 2026-09-01 by fandom.com AND nanoka.cc agreeing exactly) are consistently ~1.988× this fresh Prydwen dump's own stated values (10.00%×3+160.00%+10.00% vs. stored 19.89%×3+318.10%+19.89%, etc. — verified as a real ~1.988 ratio, not noise, across 3 independent moves). Given the existing value has 2-source-agreement from just one day before this dump's context date, this needs a human call on which source is current for Rebecca specifically (Patch 3.4 balance changes are plausible either direction) — not silently overwritten. 1 new test, full suite green (1302/1302). |
