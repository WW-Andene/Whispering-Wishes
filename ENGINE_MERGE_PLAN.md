...
| Verina | **Partial — 4 real fixes 2026-09-02 against a fresh user-pasted Prydwen text, NOT a full 8-dimension Phase A pass** | Not audited block-by-block against all 8 dimensions. `SKILL_MULTIPLIERS`/`CHARACTER_ROTATIONS` values already matched. **Fix 1 (real stat-category bug, found by cross-referencing this file's OWN tables against each other)**: Outro (Blossom)'s team buff was `stat:'allDmg'` ("confirmed Amplified" per a prior session's own note) — but the fresh Prydwen text explicitly says "15% all-Type DMG **Deepen** for 30s", and this file's own separate dmgFocus buff-tag entry (line ~1316) already said `['ATK Buff', 'DMG Deepen', 'Heal']`, directly contradicting the "confirmed Amplified" claim — an internal inconsistency the prior check missed. Fixed `CHAR_BUFF_TABLE`, `SKILL_MULTIPLIERS`' note, `CHARACTER_ROTATIONS`' note, and `verina.blocks.js` all to `deepen`. **Fix 2**: `verina.forte.starflower-blooms-midair` had no `damage.category` — kit text explicitly says "considered as Basic Attack damage" — fixed to `basicDmg`. **Fix 3 (2nd confirmed instance of the Engine development.md item 12 architecture gap)**: `verina.chain.s6` was `kind:'buff', trigger:{type:'cast',...}, timing:{}` — the same dead-buff shape found on Lupa's S4 — converted to a real `kind:'damage'` proportional-2nd-hit block (44.592% = 20% of Mid-air Starflower Blooms' own 222.96% base total). **Fix 4**: S6's Coordinated Attack proc (previously entirely unmodeled, not just the dead buff) added as its own block, `verina.chain.s6-coordinated-attack` (9.95% ATK, `coordDmg`, sourced from the same value Liberation's own Photosynthesis Mark proc uses, per S6's own kit text). 6 new/rewritten tests, full suite green (1305/1305). |
| Augusta | **Partial — 2 real fixes 2026-09-02 against a fresh user-pasted Prydwen text, NOT a full 8-dimension Phase A pass** | Not audited block-by-block against all 8 dimensions. `SKILL_MULTIPLIERS`/`CHAR_BUFF_TABLE`/`CHARACTER_ROTATIONS`/`dmgFocus` all already matched exactly (including her Outro correctly being `allDmg`/"Amplification", not a Verina-style Deepen mislabel — the kit text really does say Amplification here). S6's Thunder Rage was already correctly built as its own gated damage block from a prior session (not a gap). **Fix 1 (real over-crediting bug)**: `augusta.chain.s3`'s `totalMult:25` was a single UNSCOPED effect, with a prior session's note claiming this was safe since "her only Heavy ATK hits anyway" — wrong, since `totalMult` applies unconditionally to EVERY hit regardless of category (not category-gated), so it was silently over-crediting her real `skillDmg` hits (Warrior's Blade, Undying Sunlight: Strike/Leap) and her Intro too, none of which S3's own kit text lists (only Thunderoar: Backstep/Spinslash/Uppercut + Dodge Counter equivalents, Undying Sunlight: **Plunge** specifically — not Strike/Leap — and Sublime is the Sun: Sunborne/Everbright Protector). Fixed via `scopedToBlockId` (Phase 0.5 gap #3's mechanism) across the 6 real blocks that fire in her modeled rotation. **Fix 2**: `augusta.intro.stride-of-goldenflare` had no `damage.category` — the dump's own row label ("Skill Damage", not move-specific) confirms `skillDmg` per the same convention already established on Lupa/Ciaccona. 3 new tests (one proving the over-credited hits are now unaffected while the real 6 named moves still get boosted), full suite green (1307/1307). |
| Carlotta | **Partial — 1 real fix 2026-09-02 against a fresh user-pasted Prydwen text, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. `SKILL_MULTIPLIERS`/`RESONANCE_CHAIN_DATA`/`CHAR_BUFF_TABLE`/`CHARACTER_ROTATIONS` values already matched this Prydwen text exactly (previously audited 2026-08-31 against wutheringwaves.fandom.com); `carlotta.blocks.js` already correctly categorized Era of New Wave/Death Knell/Fatal Finale/Imminent Oblivion as `skillDmg` despite their Liberation/Heavy input slots, per their own "considered Resonance Skill DMG" kit text — no miscategorization bug found here (unlike Rebecca/Phrolova's reversed pattern). **Fix (3rd confirmed instance of the Engine development.md item 12 architecture gap)**: `carlotta.chain.s2` (Fatal Finale DMG Multiplier +126%) was `kind:'buff', trigger:{type:'cast', on:'Liberation:Fatal Finale'}, timing:{}` — the same dead cast-scoped/no-duration shape as Lupa's S4 and Verina's S6, a silent no-op in `resolveHitComposedDps.js`'s `statsAtInstant()`. Fixed via conversion to `trigger:{type:'passive'}` + `scopedToBlockId:'carlotta.liberation.fatal-finale'` (Augusta's S3 scoping pattern), so it fires and applies only to Fatal Finale's own hit. **Fixed 2026-09-02 (2nd pass, re-read after initially flagging this as unfixed too hastily)**: `carlotta.chain.s1` (+12.5% Crit Rate on DMG dealt to a Deconstructed target) has the identical dead cast-scoped/no-duration `kind:'buff'` no-op shape as S2 — a 4th confirmed instance of the item-12 architecture bug. First left unfixed on the theory that "unconditional Crit Rate" would be an unsourced guess — but the pasted text's own Review section says outright "This debuff [Deconstruction] lasts for 4 seconds but with the Inherent Ability [Ars Gratia Artis] active, it should always be active," a direct, explicit source for treating her real rotation as continuously Deconstructed. Fixed by converting to `trigger:{type:'passive'}`, unscoped (matches the sourced "always active" claim, unlike S2 which is scoped to one named move). **Fixed 2026-09-02 (2nd pass, user caught this — the numbers are literally at the top of the pasted text's own Multipliers table)**: SKILL_MULTIPLIERS was missing Mid-air Attack (104.78%), Customary Greetings (107.99%+131.99%), and Dodge Counter (103.77%+137.55%) rows — added as `['Mid-air', 'Attack', ...]`/`['Mid-air', 'Customary Greetings', ...]`/`['Dodge Counter', 'Riposte', ...]`. Still no new engine block for them since `CHARACTER_ROTATIONS`'s Mid-air step is pure repositioning (no damage focus per its own note); `Carlotta[3] "Mid-air: Plunging Attack"` correctly added to `data-integrity.test.js`'s `KNOWN_UNRESOLVED_BASELINE` (its skill string doesn't name-match either new row, by design). 2 new tests total, full suite green (1308/1308). |
| Galbrena | **Partial — 5 real fixes 2026-09-02 against a fresh user-pasted Prydwen text, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. `SKILL_MULTIPLIERS`/`RESONANCE_CHAIN_DATA`/`CHAR_BUFF_TABLE` values already matched (previously audited 2026-09-01), except one genuinely missing row. **Fix 1 (dmgFocus)**: was `['Main Damage Dealer', 'Heavy Attack Damage']`, missing 'Echo Skill Damage' despite Echo being her single LARGEST damage bucket (57.1% vs Heavy's 32.1%) per the source's own damage profile. **Fix 2 (`chain.s3`, 2 stacked bugs)**: was `trigger:{type:'cast', on:'Echo:Hellfire Absolution'}` with no `timing.duration` — 5th confirmed instance of the item-12 dead-buff architecture bug — fixed via `trigger:{type:'passive'}` + `scopedToBlockId`; ALSO `stat:'libDmg'` when its target block (`galbrena.echo.hellfire-absolution`) is `category:'echoDmg'` — category-gated stats only apply to matching-category hits, so it was independently a 2nd no-op — fixed to `echoDmg`. **Fix 3 (`chain.s4`)**: was `target:{scope:'self'}` — kit text is explicit this is team-wide ("all Resonators in the team gain 20% all-Attribute DMG Bonus"), fixed to `whole-team` (trigger anchor for "any teammate's Echo cast" kept as a documented passive/unconditional approximation, same tradeoff as her own Afterflame mechanic). **Fix 4 (`chain.s6`)**: only had a `heavyDmg` effect — the kit text names Seraphic Execution/Flamewing Verdict as whole moves, but their own Stage 4/5 and Stage 3 are `echoDmg`-categorized, so a single effect silently missed those stages — added a 2nd `echoDmg:60` effect covering the real category split. **Fix 5 (data completeness)**: SKILL_MULTIPLIERS was missing the Hellstride row (666 flat, unbuffable) — added as reference data only, no engine block (Review section confirms it's unused in her real rotation, and its standalone-flat shape doesn't fit `parseSkillMultiplierHits`' existing flat+%ATK mechanism). 5 new/rewritten tests, full suite green (1311/1311). |
| Lucy | **Partial — 9 real fixes 2026-09-02 against a real prydwen.gg .mht snapshot, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. This was the first time an unblocked .mht snapshot was available for Lucy (prior audits noted the exact per-move Multipliers tables "render client-side... not exposed in text extraction"), so several previously-undiscoverable gaps resolved. **Fix 1**: `tuneBreak.ruptureDmgMult` for Data Crash was blank/unconfirmed — now sourced: 1367.75 (1094.19%+68.39%×4). **Fix 2**: SKILL_MULTIPLIERS Basic Attack Stage 1 had a fabricated `×6` repeat — corrected to a single hit. **Fix 3**: the 'Payload / Pulse Interference / Deadlock' combined row's Payload segment was truncated with a literal `"..."` in the stored string — now the full sourced Charge+Follow-Up value, wired into `lucy.skill.payload`. **Fix 4 (biggest DPS-relevant find)**: `Basic ATK: Thread Shredding Stage 1-4` and `Heavy ATK: Dual Threading` — 2 real CHARACTER_ROTATIONS steps with NO matching SKILL_MULTIPLIERS row at all (silently 0 DMG, flagged in both `data-integrity.test.js`'s KNOWN_UNRESOLVED_BASELINE and the blocks file's own "not modeled rather than guessed" comment) — now added with real sourced values and wired as new engine blocks, a real DPS undercount fix in her actual burst rotation. **Fix 5**: `chain.s1` was an unconditional passive +20% ATK — kit text says it's a 14s window on Intro cast — converted to a real cast-scoped buffWindow. **Fix 6 (6th/7th confirmed instances of the item-12 dead-buff bug)**: `chain.s2` and `chain.s3` were both `kind:'buff', trigger:{type:'cast',...}, timing:{}` — fixed via passive+scopedToBlockId. **Fix 7**: `chain.s3`'s `libDmg:50` was ALSO independently a category-gating no-op (its target block is `category:'heavyDmg'`) — fixed to `heavyDmg`, the 3rd character (after Carlotta/Galbrena) found with this exact double-bug shape. **Fix 8**: `chain.s4`'s duration was 25 — conflated with the Outro's own separate 25s window — kit text says 20s, fixed. **Fix 9**: `chain.s5` (`totalMult:5`) had zero textual basis (S5's real text is 100% defensive) — zeroed in both RESONANCE_CHAIN_DATA and the engine block, matching Augusta's S5 precedent. **Found, not fixed**: S6 also grants +60% Hack DMG with no home in this schema's damage-category set — documented gap. 7 new/rewritten tests, full suite green (1316/1316). |
| Rover: Aero | **Partial — 7 real fixes 2026-09-02 against a real prydwen.gg .mht snapshot, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. SKILL_MULTIPLIERS/RESONANCE_CHAIN_DATA values already matched exactly. **Fix 1**: `bestWeapon` was 'Emerald of Genesis' — not mentioned anywhere in the source (which compares ONLY her own signature, Bloodpact's Pledge) — fixed. **Fix 2**: `bestEchoes` named 'Gusts of Welkin 5pc' (unsourced) and 'Reminiscence: Fleurdelys' as primary, when the real best set is Rejuvenating Glow with Bell-Borne Geochelone as its main echo (Fleurdelys is the alt-set's main echo) — fixed. **Fix 3**: `teams` named 'Rover: Aero + Jinhsi + Shorekeeper' — Jinhsi never appears in the source; real teams are Cartethyia+Ciaccona (best) and Iuno+Ciaccona (Main DPS Iuno) — fixed. **Fix 4**: `CHAR_BUFF_TABLE`'s `outroBuffs`/`libBuffs` both encoded real non-DPS mechanics (an Aero Erosion stack-cap increase, a healing formula) under a fabricated `totalMult` key — zeroed (the engine already avoided using them; real mechanics moved to the note field). **Fix 5**: `chain.s4` was an unconditional passive — kit text says it's a 5s window on Cloudburst Dance cast — converted to a real cast-scoped buffWindow. **Fix 6 (8th confirmed instance of the item-12 dead-buff bug)**: `chain.s5` was `kind:'buff', trigger:{type:'cast',...}, timing:{}` — fixed via passive+scopedToBlockId. **Fix 7**: `chain.s6` was an unscoped passive `skillDmg:30`, over-crediting her other Resonance-Skill-DMG-categorized moves instead of only Unbound Flow (its own kit text's real scope) — fixed via `scopedToBlockId` (Augusta's S3 pattern). **Fix 8, added 2026-09-03 (missed originally, caught while fixing the same gap on Brant)**: `roveraero.midair.plunging-attack` had no `damage.category` — fixed to `basicDmg` per the established Mid-air Attack convention, closing the cross-character Mid-air sweep for this character. 8 new/rewritten tests, full suite green (1322/1322). |
| Brant | **Partial — 4 real fixes 2026-09-02 against a real prydwen.gg .mht snapshot, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. SKILL_MULTIPLIERS/RESONANCE_CHAIN_DATA/CHAR_BUFF_TABLE values already matched exactly (including the already-logged S2 Outro-proc gap, matching this source word-for-word). **Fix 1**: `weaponAlts.alt4` named 'Overture'/'Commando of Conviction', neither mentioned anywhere in the source's exhaustive 5-weapon list (signature + 4 real alts, no 4-star at all) — the 2 real missing alt5s (Red Spring, Emerald of Genesis) moved there, alt4 cleared. **Fix 2**: `brant.midair.stage-2-3-charged-flip` had no `damage.category` — fixed to `basicDmg` per the established Mid-air Attack convention. **Fix 3/4 (2 more instances of the Augusta-S3 over-crediting shape)**: `chain.s3` (Returned from Ashes DMG Mult +42%) and `chain.s6` (Mid-air Attack DMG Mult +30%) were BOTH single UNSCOPED `totalMult` effects, silently boosting his entire kit instead of only their real named target — fixed via `scopedToBlockId` on both (S3 scoped to Returned from Ashes + its S6 secondary blast; S6 scoped to the Mid-air Attack block). Also added missing reference-only SKILL_MULTIPLIERS rows for base Heavy Attack/Plunging Attack/Dodge Counter (confirmed unused in his real rotation). 7 new/rewritten tests, full suite green (1322/1322). |
| Jiyan | **Partial — 2 real fixes 2026-09-03 against a real prydwen.gg .mht snapshot, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. SKILL_MULTIPLIERS/RESONANCE_CHAIN_DATA/bestWeapon/bestEchoes/weaponAlts all already matched this source exactly. **Fix 1**: `jiyan.outro.discipline` had no `damage.category` — the kit text explicitly calls it a "Coordinated Attack", mapping directly to this schema's `coordDmg` category — fixed. **Fix 2 (new variant of the item-12 dead-buff bug)**: `chain.s5-outro-mult` (Outro Discipline DMG Mult +120%) was `trigger:{type:'swap-out'}` with no `timing.duration` — every prior instance this session used a `'cast'` trigger, but the same root cause (any non-passive trigger with no duration is invisible to `statsAtInstant()`) applies regardless of trigger type — fixed via passive+scopedToBlockId. Also fixed `chain.s6`'s identical `'cast'`-shaped instance for correctness, though it currently has zero live impact (no Finale damage block exists to scope to — Finale is never cast in the real rotation). 3 new/rewritten tests, full suite green (1325/1325). |
| Phoebe | **Partial — 8 real fixes 2026-09-03 against a real prydwen.gg .mht snapshot, biggest single-character finding this session, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. SKILL_MULTIPLIERS/RESONANCE_CHAIN_DATA/bestWeapon/bestEchoes/weaponAlts all already matched exactly. **Fixes 1-6 (11th-16th confirmed instances of the item-12 dead-buff bug)**: 6 separate blocks — `phoebe.kit.dawn-of-enlightenment-absolution-mult` (+255% Liberation), `phoebe.kit.attentive-heart-absolution-mult` (+255% Outro), `phoebe.kit.starflash-frazzle-amp` (+256% Starflash Frazzle Amp), `chain.s1` (+225% Liberation additional), `chain.s2` (+120% Outro Frazzle Amp), `chain.s3` (+91% Starflash) — were ALL `kind:'buff'` with a non-passive trigger and no `timing.duration`, together covering nearly her ENTIRE multiplier stack. Fixed via passive+scopedToBlockId (S3 didn't need scopedToBlockId, category-gated already). This was silently understating her real computed DPS by a massive margin. **Fix 7**: SKILL_MULTIPLIERS was missing the Utter Confession row (187.9%) — added as reference data (her modeled rotation stays Absolution-only). **Fix 8**: `CHAR_BUFF_TABLE`'s Confession Frazzle-stack count was 18 with no shown derivation — recomputed from the source's own rotation (1 Utter Confession + 8 Liberation + 5×2 Starflash = 19) and corrected. 9 new/rewritten tests, full suite green (1327/1327). |
| Zani | **Partial — 5 real fixes 2026-09-03 against a real prydwen.gg .mht snapshot, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. RESONANCE_CHAIN_DATA/CHAR_BUFF_TABLE/bestWeapon/bestEchoes/weaponAlts all already matched exactly. **Fix 1**: 4 rotation-lookup gaps already flagged in data-integrity.test.js's KNOWN_UNRESOLVED_BASELINE — Standard Defense Protocol (no row at all), Targeted Action / Forcible Riposte (row name too short to substring-match), the 3 Heavy Slash rows (missing colons vs. the real rotation step names), and Basic ATK: Stage 3 (never matched the combined Stage 1-4 row) — all fixed with real sourced data, baseline entries removed. **Fix 2**: resolved a long-standing TODO — Daybreak/Dawning's exact Blaze costs (10/20) are now explicitly confirmed by the source's Review text. **Fix 3**: `zani.skill.targeted-action` had no `damage.category` — fixed to `skillDmg`, which also unblocked chain.s2's own skillDmg effect. **Fix 4 (17th-19th confirmed instances of the item-12 dead-buff bug)**: `chain.s2`/`s3`/`s5` were all `kind:'buff', trigger:{type:'cast',...}, timing:{}` — fixed via passive+scopedToBlockId (S3/S5 both needed scoping since `libDmg` is shared between Rekindle and The Last Stand). **Fix 5**: `teams` led with 'Zani + Phoebe + Shorekeeper' — source explicitly calls Phoebe+Spectro Rover "the best Zani team when fully optimized" — reordered. 7 new/rewritten tests, full suite green (1329/1329). |
| Mortefi | **Partial — 1 real fix 2026-09-03 against a real prydwen.gg .mht snapshot, NOT a full 8-dimension Phase A pass** | No dump file existed before this pass — created. SKILL_MULTIPLIERS/RESONANCE_CHAIN_DATA/CHAR_BUFF_TABLE/CHARACTER_ROTATIONS/bestWeapon/bestEchoes all already matched exactly — this character had already been carefully re-audited 2026-09-01, including a thoughtful S1/S5 bonus-Marcato proc treatment (matching Yinlin's S6/Calcharo's S6 "discrete proc, not modifier" precedent). No dead cast-scoped/no-duration buff-block instances found here — every buff block already had a real timing.duration. **Fix**: `chain.s3` (Marcato Crit DMG +30% during Burning Rhapsody) had a single UNSCOPED `critDmg` effect — critDmg isn't category-gated, so without scoping it would over-credit ANY of Mortefi's own hits landing within the 10s buff window, when the kit text is explicit this is Marcato-only. Fixed via `scopedToBlockId` on both real Marcato proc blocks (Augusta S3/Brant S3&S6 pattern) — currently latent (nothing else casts in that window in the modeled rotation) but a real correctness fix. Also fixed a minor weaponAlts.alt4 ordering discrepancy. 2 new/rewritten tests, full suite green (1330/1330). |
| *(remaining 37 characters)* | Not started | — |
...
`basicDmg`. Brant's `brant.midair.stage-2-3-charged-flip` (fixed 2026-09-02, as part of his Brant pass)
and Rover: Aero's `roveraero.midair.plunging-attack` (fixed 2026-09-03 — missed during the original
2026-09-02 Rover: Aero pass, caught while fixing Brant's identical gap) now both confirmed `basicDmg`
too. This sweep is now closed for every character in the roster that has a dump to check against.
